	...
	elseif GetNumText() == 3 then
		...
		if bagpos ~= -1	then
			...
			if TicketMoney ~= 0 then
				...
				--AddMoneyJZ(sceneId, selfId, TicketMoney * x805012_g_PlayerRate*merchandiseDayRate)
				
				--x805012_MyNotifyTip( sceneId, selfId, "Các hÕ nh§n ðßþc #{_EXCHG"..TicketMoney * x805012_g_PlayerRate*merchandiseDayRate.. "}" )
				...
				BroadMsgByChatPipe( sceneId, selfId, "@*;SrvMsg;GLD:#Y#{_INFOUSR" .. name .. "} hoàn t¤t nhi®m vø thß½ng nhân, giúp b±n bang tång s¯ ti«n cüa bang hµi lên #{_MONEY"..TicketMoney*x805012_g_GuildRate*merchandiseDayRate*FatigueRate .. "}", 6 )
