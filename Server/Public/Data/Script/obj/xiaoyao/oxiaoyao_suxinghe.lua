...
function x014000_OnDefaultEvent( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		AddText(sceneId,"  Ta là Tô Tinh Hà, quy«n chß·ng môn cüa phái Tiêu Dao, ta ðang giúp sß phø tìm kiªm chß·ng môn th§t sñ cüa phái Tiêu Dao")
...
function x014000_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetNumText()==0	then
		...
		BeginEvent(sceneId)
			...
			AddNumText(sceneId, x014000_g_scriptId, "Ta mu¯n bái nh§p phái Tiêu Dao",6,3)
	...
	if GetNumText()==3	then
		...
		elseif GetLevel( sceneId, selfId ) < 10 then
			...
			if x014000_g_MenPai == 8 then
				...
				EndEvent(sceneId)
				DispatchEventList(sceneId,selfId,targetId)
			--Tr· v«ÖµÎª9±íÊ¾ÎÞÃÅÅÉ
			elseif x014000_g_MenPai==9	then
				LuaFnJoinMenpai(sceneId, selfId, targetId, 8)

				-- ÉèÖÃ³õÊ¼toÕ ðµ Npc¹ØÏµÖµ
				CallScriptFunction( 200099, "InitRelation", sceneId, selfId )

				-- °ÑÏà¹ØtoÕ ðµ ÐÄ·¨ÉèÖÃÎª10c¤p±ð  49,52,53
				...
				x014000_MsgBox( sceneId, selfId, "L¤y ðßþc 20 t¤m Chiêu t§p l®nh cüa môn phái" )

				if TryRecieveItem( sceneId, selfId, 10124007, 1 ) >= 0 then
					str		= "#YCác hÕ ðã nh§n ðßþc #W"..GetItemName( sceneId, 10124007 ).."."
	...
	if GetNumText()==6 then
		...
		EndEvent(sceneId)
		DispatchEventList(sceneId, selfId, targetId)
		CallScriptFunction( SCENE_SCRIPT_ID, "AskTheWay", sceneId, selfId, sceneId, 125, 41, "¿µ¹ãÁê" )
		return
	end
end
