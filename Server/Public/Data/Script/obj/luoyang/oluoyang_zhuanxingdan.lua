...
function x000147_OnZhuanXingRequest( sceneId, selfId, targetId )
	BeginEvent(sceneId)
		...
		AddNumText( sceneId, x000147_g_scriptId, "Hüy", 6, 0147004)		
	EndEvent(sceneId)
	DispatchEventList(sceneId,selfId,targetId)
end
...
function x000147_OnZhuanXingConfirm( sceneId, selfId, targetId, sex, hairColor, hairModel, faceModel, nFaceId)
	...
	if pre_condition == 0 then
		...
		if is_suc == 1 then
			...
			local strText
			if sex == 0 then
				strText = format("#{_INFOUSR%s} #cff66ccÐã dùng 1 cái Chuy¬n Tính Ðan, biªn ð±i nhân dÕng thành næ nhân...", playername)
			end
			if sex == 1 then
				strText = format("#{_INFOUSR%s} #c0066ffÐã dùng 1 cái Chuy¬n Tính Ðan, biªn ð±i nhân dÕng thành nam nhân...", playername)
			end
