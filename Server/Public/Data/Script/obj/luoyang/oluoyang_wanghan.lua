...
x000128_g_Impact_NotTransportList = { 5929, 5944 } -- ½ûÖ¹´«ËÍtoÕ ðµ Impact
x000128_g_TalkInfo_NotTransportList = { "#{GodFire_Info_062}", "#{XSHCD_20080418_099}" } -- ½ûÖ¹´«ËÍtoÕ ðµ ImpactÌáÊ¾ÐÅÏ¢
...
function x000128_OnDefaultEvent( sceneId, selfId,targetId )
	...
	BeginEvent( sceneId )
		...
		AddNumText( sceneId, x000128_g_ScriptId, "Thành Th¸ - Tô Châu - Thiªt Tßþng Ph¯", 9, 1007 )
		...
		AddNumText( sceneId, x000128_g_ScriptId, "#GThành Th¸ - Phøng Minh Tr¤n", 9, 1013 )
		--AddNumText( sceneId, x000128_g_ScriptId, "Thành ph¯  - LÕc Dß½ng", 9, 1006 )
		AddNumText( sceneId, x000128_g_ScriptId, "Ðßa ta ðªn nhæng môn phái khác", 9, 1012 )
...
function x000128_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetTeamId(sceneId,selfId)>=0 and 
		...
		for	i=0,num-1 do
			...
			if IsHaveMission(sceneId,mems[i],4021) > 0 then
				x000128_MsgBox( sceneId, selfId, targetId, "  Trong ðµi ngû cüa các hÕ có ngß¶i ðang làm nhi®m vø v§n chuy¬n, thß½ng nhân ta không th¬ cung c¤p d¸ch vø cho các hÕ ðßþc." )
				return
			end
		end
	end
	...
	if IsHaveMission(sceneId,selfId,4021) > 0 then
		x000128_MsgBox( sceneId, selfId, targetId, "  Xin thÑ l²i! Các hÕ ðang mang trong mình nhi®m vø v§n chuy¬n, thß½ng nhân ta không th¬ cung c¤p d¸ch vø cho các hÕ." )
		return
	end
	...
	if arg == 1000 then		--Tr· v«ÃÅÅÉ
		if id < 0 or id >= 9 then
			x000128_MsgBox( sceneId, selfId, targetId, "  Các hÕ chßa gia nh§p môn phái nào." )
		else
