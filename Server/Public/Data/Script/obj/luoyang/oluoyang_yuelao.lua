...
--Ä¿±êNPC
x000093_g_name	="Nguy®t Lão"

x000093_StartTime   = 9040
x000093_EndTime     = 9047

--ËùÓµÓÐtoÕ ðµ ÊÂ¼þIDÁÐ±í {½á»é, Ñ§Ï°·òÆÞ¼¼ÄÜ, Àë»é, Ç¿ÖÆÀë»é, ¹ºÂòÇëÌû,·òÆÞÎÊ´ð}
x000093_g_RelationEventList={806003, 806016, 806005, 806004, 806017,888901, 808010} --»é½äÉýc¤p 808010 zchw
...
function x000093_UpdateEventList( sceneId, selfId, targetId )
	BeginEvent(sceneId)
		...
		AddNumText( sceneId, x000093_g_scriptId, "Liên quan tâm hæu linh tê", 11, 11 )
		
		local curDayTime = GetDayTime()
		if (curDayTime>=x000093_StartTime and curDayTime<x000093_EndTime) then
			AddNumText(sceneId,x000093_g_scriptId,"V« Vînh H¢ng Toàn Gi¾i",11,12)
		end
