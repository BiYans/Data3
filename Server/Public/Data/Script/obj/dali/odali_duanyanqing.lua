...
function x002016_OnDefaultEvent( sceneId, selfId,targetId )
	--x002016_NotifyTips(sceneId,selfId,GetMissionData(sceneId,selfId,LUAAUDIT_FEIHUANGSHI))
	--x002016_NotifyTips(sceneId,selfId,GetMissionData(sceneId,selfId,LUAAUDIT_FEIHUANGSHI_BOUND))
...
function x002016_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if num == 1 then										-- Nhi®m vø Ðoàn Diên Khánh
		...
		--¿´ÏÂ½ñÌìtoÕ ðµ ¶ÎÑÓÇìÈÎÎñÐúng²»Ðúng×öÍê50´ÎÁË
...
function x002016_NPCTalkOnFirstSubmission( sceneId, selfId, missionIndex, isDone)--[tx43452]
  ...
  local adCount = mod(countValue, x002016_g_cdLimid)--¸ø·É»ÈÊ¯toÕ ðµ ´ÎÊý
	local msCount = floor(countValue / x002016_g_cdLimid)--Íê³É¹ØÏµÖµ¸ßÓÚµÈÓÚ480toÕ ðµ ´ÎÊý 
...
--**********************************
--¼ì²éÐúng·ñThöa mãn°µÆ÷·Å³öÇ°ÌáÌõ¼þ
--**********************************
...
function x002016_OnMissionSubmitionSuccess(sceneId, selfId, targetId, missionIndex)--[tx43452]
	...
	local adCount = mod(countValue, x002016_g_cdLimid)--¸ø·É»ÈÊ¯toÕ ðµ ´ÎÊý
	local msCount = floor(countValue / x002016_g_cdLimid)--Íê³É¹ØÏµÖµ¸ßÓÚµÈÓÚ480toÕ ðµ ´ÎÊý
	AddGlobalCountNews(sceneId,msCount)
	msCount = msCount + 1
	if msCount == 1 then--·Å³ömµt cái°ó¶¨toÕ ðµ ·É»ÈÊ¯
	  local bagIndex = x002016_RecevieRewards(sceneId, selfId, targetId, x002016_g_BoundRewardsId)
		...
		local testCount = 50 * x002016_myPower( 2, adCount - 1 )--¼ÆÊý´Ó¹ØÏµÖµ>=480¿ªÊ¼¼Ç,ÕâÀïÓÃ50*2^(n-1),
		AddGlobalCountNews(sceneId,testCount)
		AddGlobalCountNews(sceneId,msCount)
		if testCount == msCount  then--msCount·Å³ömµt cái·Ç°ó¶¨toÕ ðµ ·É»ÈÊ¯
...
function x002016_RecevieRewards(sceneId, selfId, targetId, ItemId)--[tx43452]
	--Êµ¼ÊTÕi Ç°ÃæÒÑ·ÅÈë±³°ü,ÕâÀïÖ»ÌáÊ¾³É¹¦
