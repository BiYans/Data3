...
x002084_g_NewCardList[x002084_g_NewCard4] = {
	...
	recvMsg = "    — ðây t£ng cho các hÕ 2000 nguyên bäo khóa.",
}
x002084_g_NewCardList[x002084_g_NewCard5] = {
	...
	recvMsg = "    — ðây t£ng cho các hÕ 4000 nguyên bäo khóa.",
}
...
x002084_g_NewCardList[x002084_g_NewCard7] = {
	...
	recvMsg = "    — ðây t£ng cho các hÕ 6000 nguyên bäo khóa.",
}
x002084_g_NewCardList[x002084_g_NewCard8] = {
	...
	recvMsg = "    — ðây t£ng cho các hÕ 7520 nguyên bäo khóa.",
}
...
function x002084_OnDefaultEvent( sceneId, selfId, targetId )
	BeginEvent( sceneId )
		...
		--AddNumText( sceneId, x002084_g_scriptId, "Ð±i phiªu ði¬m t£ng", 7, 131 )
...
function x002084_OnEventRequest( sceneId, selfId, targetId, eventId )
  ...
  for i, findId in x002084_g_eventList do
	...
	elseif GetNumText() == 131 then
		...
		BeginEvent(sceneId)
		  AddText(sceneId, "C¤p 50 tr· lên m¾i có th¬ sØ døng chÑc nång này")
		EndEvent(sceneId)
		DispatchEventList( sceneId, selfId, targetId )
		return
		end
...
function x002084_CheckNewUserCard( sceneId, selfId, targetId )
	...
	if nItemNum > 0 then
		...
		--´ò¿ªÊäÈë²Æ¸»¿¨¿¨ºÅ½çÃæ
	  x002084_NotifyFailTips( sceneId, selfId, "Ngß½i không sÞ hæu Thë Tài Phú." )
...
function x002084_GetAward( sceneId, selfId, targetId, grade )
	...
	if GetMissionFlag( sceneId, selfId, MF_GetAwardFlag ) ~= 1 then
		x002084_NotifyFailBox( sceneId, selfId, targetId, "     th¶i ngß½i không th¬ lînh ðßþc ph¥n thß·ng" )
		return
	end
...
function x002084_GetNewCardPrize( sceneId, selfId, targetId, grade )
	...
	if GetMissionFlag( sceneId, selfId, MF_ActiveNewUserCard ) ~= 1 then
		x002084_NotifyFailBox( sceneId, selfId, targetId, "    TÕm th¶i ngß½i không th¬ lînh ðßþc ph¥n thß·ng" )
		return
	end
	...
	if awardInfo.moneyReward > 0 then
		if grade == x002084_g_NewCard4 or grade == x002084_g_NewCard5 or grade == x002084_g_NewCard6 or grade == x002084_g_NewCard7 or grade == x002084_g_NewCard8 then
			...
			local NBK = GetMissionData(sceneId,selfId,YUANBAOLOCK) + awardInfo.moneyReward;
			SetMissionData(sceneId,selfId,YUANBAOLOCK,NBK);
			CallScriptFunction( 830001, "YuanBaoLockSendUI", sceneId, selfId )
		else
			AddMoney( sceneId, selfId, awardInfo.moneyReward )
		end
	end
