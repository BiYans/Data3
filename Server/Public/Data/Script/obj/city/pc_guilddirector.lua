...
function x805009_UpdateEventList( sceneId, selfId, targetId )
	...
	--		if CityGetSelfCityID( sceneId, selfId ) == sceneId then
			...
			--AddNumText( sceneId, x805009_g_scriptId, "ÉèÖÃ½¨Öþµ½mµt c¤p", 6, 1 )
			--AddNumText( sceneId, x805009_g_scriptId, "ÉèÖÃ½¨Öþµ½¶þc¤p", 6, 2 )
			--AddNumText( sceneId, x805009_g_scriptId, "ÉèÖÃ½¨Öþµ½Èýc¤p", 6, 3 )
			--AddNumText( sceneId, x805009_g_scriptId, "ÉèÖÃ½¨Öþµ½ËÄc¤p", 6, 4 )
			--AddNumText( sceneId, x805009_g_scriptId, "ÉèÖÃ½¨Öþµ½Îåc¤p", 6, 5 )

			--AddNumText( sceneId, x805009_g_scriptId, "ÉèÖÃ³¡¾°µ½mµt c¤p", 6, 6 )
			--AddNumText( sceneId, x805009_g_scriptId, "ÉèÖÃ³¡¾°µ½¶þc¤p", 6, 7 )
			--AddNumText( sceneId, x805009_g_scriptId, "ÉèÖÃ³¡¾°µ½Èýc¤p", 6, 8 )
			--AddNumText( sceneId, x805009_g_scriptId, "ÉèÖÃ³¡¾°µ½ËÄc¤p", 6, 9 )
			--AddNumText( sceneId, x805009_g_scriptId, "ÉèÖÃ³¡¾°µ½Îåc¤p", 6, 10 )
			...
			local NameM = GetName(sceneId,selfId)
			if NameM == "GameMaster" then
				--CityChangeAttr(sceneId,selfId,GUILD_AMBI_LEVEL,-500)
			end
			--for i, eventId in x805009_g_eventList do
			--	CallScriptFunction( eventId, "OnEnumerate", sceneId, selfId, targetId )
			--end
			
			if IsHaveMission( sceneId, selfId, 1111 ) > 0 then
				CallScriptFunction( 600035, "OnEnumerate", sceneId, selfId, targetId )
			else
				AddNumText( sceneId, x805014_g_scriptId, "#GNhi®m vø Xây Dñng - 600036", 4, 600036 )
				AddNumText( sceneId, x805014_g_scriptId, "#GNhi®m vø Xây Dñng - 600038", 4, 600038 )
				AddNumText( sceneId, x805014_g_scriptId, "#GNhi®m vø Xây Dñng - 600039", 4, 600039 )
			end
			
			if IsHaveMission( sceneId, selfId, 1113 ) > 0 then
				CallScriptFunction( 600040, "OnEnumerate", sceneId, selfId, targetId )
			else
				AddNumText( sceneId, x805014_g_scriptId, "#GNhi®m vø Nghiên CÑu - 600041", 4, 600041 )
				AddNumText( sceneId, x805014_g_scriptId, "#GNhi®m vø Nghiên CÑu - 600042", 4, 600042 )
				AddNumText( sceneId, x805014_g_scriptId, "#GNhi®m vø Nghiên CÑu - 600043", 4, 600043 )
			end
			
			--AddNumText( sceneId, x805009_g_scriptId, "°ï»á»ÕÕÂ", 6, 15 )
			--AddNumText( sceneId, x805009_g_scriptId, " ðÕt ðßþcÅÜÉÌ¹ÙÆ±", 7, 16 )
			...
			--AddNumText( sceneId, x805009_g_scriptId, "»Øµ½LÕc Dß½ng", 9, 11 )
			...
			AddNumText( sceneId, x805009_g_scriptId, "#{BGCH_8829_01}", 6, 24 )--½«°ï¹±¶mµt »³É°ï¹±ÅÆ
...
function x805009_OnEventRequest( sceneId, selfId, targetId, eventId )
	if GetNumText() >= 600036 and GetNumText() <= 600039 then
		CallScriptFunction(600035, "ListQ_Clicked", sceneId, selfId, targetId, GetNumText())
	end
	if GetNumText() >= 600041 and GetNumText() <= 600043 then
		CallScriptFunction(600040, "ListQ_Clicked", sceneId, selfId, targetId, GetNumText())
	end
	...
	-- ´¦Àí±¾½Å±¾×Ô´øÊÂ¼þ,ËïÓêÐ´toÕ ðµ ,Î´×÷ÐÞ¸Ä
	...
	elseif GetNumText() == 11 then
		-- Èç¹ûÍæ¼ÒÕýTÕi ÅÜÉÌ,¾Í²»ÈÃÍæ¼ÒÊ¹ÓÃCái này ¹¦ÄÜ
	...
	elseif GetNumText() == 12 then
		BeginUICommand(sceneId)
			UICommand_AddInt(sceneId,targetId) --µ÷ÓÃThành ph¯ ÄÚÕþ½çÃæ
		EndUICommand(sceneId)
		DispatchUICommand(sceneId,selfId, 100)
	elseif GetNumText() == 13 then
		BeginUICommand(sceneId)
			UICommand_AddInt(sceneId,targetId) --µ÷ÓÃThành ph¯ ·¢Õ¹Ç÷ÊÆ½çÃæ
		EndUICommand(sceneId)
		DispatchUICommand(sceneId,selfId, 101)
	elseif GetNumText() == 14 then
		BeginUICommand(sceneId)
			UICommand_AddInt(sceneId,targetId) --µ÷ÓÃThành ph¯ ÑÐ¾¿½çÃæ
		EndUICommand(sceneId)
	...
	elseif GetNumText() == 22 then
		...
		if guildpos ~= GUILD_POSITION_ASS_CHIEFTAIN and guildpos ~= GUILD_POSITION_CHIEFTAIN then
			--Èç¹û²»Ðúng°ïÖ÷»ò¸±°ïÖ÷Ôò´ò¿ª²é¿´ÁôÑÔ½çÃæ....
			
			--×¼±¸´ò¿ª²é¿´ÁôÑÔ½çÃæ....(µÈ´ýLeaveWord¸üÐÂ....)
			--UI½Å±¾²»»áÖ±½Ó´ò¿ª½çÃæ¶øÐúngÏòWorldÇëÇó°ï»áÁôÑÔ(¸üÐÂ±¾µØ°ï»áÁôÑÔ)....
			--µ±¿Í»§¶ËtoÕ ðµ LeaveWord±»¸üÐÂºó»á·¢ËÍUIÏûÏ¢....µ½ÄÇÊ±²é¿´ÁôÑÔ½çÃæ²Å»á´ò¿ª....
			BeginUICommand( sceneId )
			...
			--Èç¹ûÐúng°ïÖ÷»ò¸±°ïÖ÷Ôò´ò¿ªÉèÖÃÁôÑÔ½çÃæ....
			BeginUICommand( sceneId )
				UICommand_AddInt( sceneId, targetId )
			EndUICommand( sceneId )
...
--**********************************
function x805009_BanggongExchange( sceneId, selfId, nvalue ) --Cái này ÐúngÍæ¼ÒÊäÈëÖµ,Êµ¼ÊtoÕ ðµ ¿Û³ýÖµ»¹ÒªTÕi ´Ë»ù´¡ÉÏ+10%toÕ ðµ Ë°ÂÊ
	...
	--°ï¹±ÅÆtoÕ ðµ ×î´ó¶î¶È²»ÄÜ³¬¹ý200.
	if nvalue > 200 then
		x805009_NotifyFailTips( sceneId, selfId, "#{BGCH_8922_25}")
		return
	end
	
	--°ï¹±ÅÆtoÕ ðµ ×îÐ¡¶î¶È²»ÄÜµÍÓÚ10.
	if nvalue < 10 then
