...
function x805008_UpdateEventList( sceneId, selfId, targetId )
	...
	BeginEvent( sceneId )
		if Humanguildid == cityguildid then
			...
			if IsHaveMission( sceneId, selfId, 1105 ) > 0 then
				CallScriptFunction( 600002, "OnEnumerate", sceneId, selfId, targetId )
			else
				AddNumText( sceneId, x805008_g_scriptId, "#GNhi®m vø Công Nghi®p - 600003", 4, 600003 )
				AddNumText( sceneId, x805008_g_scriptId, "#GNhi®m vø Công Nghi®p - 600004", 4, 600004 )
				AddNumText( sceneId, x805008_g_scriptId, "#GNhi®m vø Công Nghi®p - 600005", 4, 600005 )
				AddNumText( sceneId, x805008_g_scriptId, "#GNhi®m vø Công Nghi®p - 600006", 4, 600006 )
			end
			
			AddNumText( sceneId, x805008_g_scriptId, "C¯ng hiªn ði¬m #GCông Nghi®p", 6, 7413 )
...
function x805008_OnEventRequest( sceneId, selfId, targetId, eventId )
	if GetNumText() >= 600003 and GetNumText() <= 600006 then
		CallScriptFunction(600002, "ListQ_Clicked", sceneId, selfId, targetId, GetNumText())
	end
	...
	if GetNumText() == 7413 then
		BeginEvent(sceneId)
			AddText( sceneId, "Ngß½i th§t sñ mu¯n dùng ði¬m c¯ng hiªn ð¬ gia tång ðµ Công Nghi®p cüa bang hµi?" )
			AddNumText( sceneId, x805008_g_scriptId, "Ðúng, ta ðã sÇn sàng!", 6, 74131 )
			AddNumText( sceneId, x805008_g_scriptId, "Hüy...", 6, 74130 )
		EndEvent(sceneId)
		DispatchEventList( sceneId, selfId, targetId )
	end
	...
	if GetNumText() == 74131 then
		...
		EndEvent(sceneId)
		DispatchEventList( sceneId, selfId, targetId )
		CityChangeAttr(sceneId,selfId,GUILD_IND_LEVEL,10)
		CityChangeAttr(sceneId,selfId,GUILD_CONTRIB_POINT,-10)
		LuaFnSendSpecificImpactToUnit(sceneId,selfId,selfId,selfId,18,0)
		local PlayerName = GetName(sceneId,selfId)
		local Str = format("@*;SrvMsg;GLD:#{_INFOUSR%s}#Y hªt lòng cho bang hµi, mang 10 ði¬m c¯ng hiªn ðªn ð¬ gia tång #GCông Nghi®p#Y, th§t ðáng ngßÞng mµ!", PlayerName)
