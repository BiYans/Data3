	...
	BeginEvent( sceneId )
		if Humanguildid == cityguildid then
			...
			if IsHaveMission( sceneId, selfId, 1106 ) > 0 then
				...
				AddNumText( sceneId, x805014_g_scriptId, "#GNhi®m vø Nông Nghi®p - 600008", 4, 600008 )
				AddNumText( sceneId, x805014_g_scriptId, "#GNhi®m vø Nông Nghi®p - 600009", 4, 600009 )
				AddNumText( sceneId, x805014_g_scriptId, "#GNhi®m vø Nông Nghi®p - 600010", 4, 600010 )
				AddNumText( sceneId, x805014_g_scriptId, "#GNhi®m vø Nông Nghi®p - 600011", 4, 600011 )
			end
			
			AddNumText( sceneId, x805014_g_scriptId, "C¯ng hiªn ði¬m #GNông Nghi®p", 6, 7413 )
...
function x805014_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetNumText() == 7413 then
		BeginEvent(sceneId)
			AddText( sceneId, "Ngß½i th§t sñ mu¯n dùng ði¬m c¯ng hiªn ð¬ gia tång ðµ Nông Nghi®p cüa bang hµi?" )
			AddNumText( sceneId, x805014_g_scriptId, "Ðúng, ta ðã sÇn sàng!", 6, 74131 )
			AddNumText( sceneId, x805014_g_scriptId, "Hüy...", 6, 74130 )
		EndEvent(sceneId)
		DispatchEventList( sceneId, selfId, targetId )
	end
	...
	if GetNumText() == 74131 then
		if CityGetAttr(sceneId, selfId, 6) <= 9 then
			...
			local Str = format("@*;SrvMsg;GLD:#{_INFOUSR%s}#Y hªt lòng cho bang hµi, mang 10 ði¬m c¯ng hiªn ðªn ð¬ gia tång #GNông Nghi®p#Y, th§t ðáng ngßÞng mµ!", PlayerName)
