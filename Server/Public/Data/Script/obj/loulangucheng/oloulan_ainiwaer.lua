...
x001100_g_Impact_NotTransportList = { 5929, 5944 } -- ½ûÖ¹´«ËÍtoÕ ðµ Impact
x001100_g_TalkInfo_NotTransportList = { "#{GodFire_Info_062}", "#{XSHCD_20080418_099}" } -- ½ûÖ¹´«ËÍtoÕ ðµ ImpactÌáÊ¾ÐÅÏ¢
...
function x001100_OnDefaultEvent( sceneId, selfId,targetId )
	...
	if GetItemCount(sceneId, selfId, x001100_g_Yinpiao)>=1  then
		BeginEvent( sceneId )
			AddText( sceneId, "  Xin thÑ lôi! trên ngß½i các hÕ ðang giæ ngân phiªu. Ta không th¬ giúp các hÕ." )
		EndEvent( sceneId )
	...
	BeginEvent( sceneId )
		...
		AddNumText( sceneId, x001100_g_ScriptId, "#GThành Th¸ - Phøng Minh Tr¤n", 9, 1013 )
		 
		AddNumText( sceneId, x001100_g_ScriptId, "Ðßa ta ðªn các môn phái khác", 9, 1011 )
...
function x001100_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetTeamId(sceneId,selfId)>=0 and 
		...
		for	i=0,num-1 do
			...
			if IsHaveMission(sceneId,mems[i],4021) > 0 then
				x001100_MsgBox( sceneId, selfId, targetId, "  Trong ðµi ngû cüa các hÕ có ngß¶i ðang làm nhi®m vø v§n chuy¬n, thß½ng nhân ta không th¬ cung c¤p d¸ch vø cho các hÕ ðßþc." )
				return
			end
		end
	end
	...
	if IsHaveMission(sceneId,selfId,4021) > 0 then
		x001100_MsgBox( sceneId, selfId, targetId, "  Xin thÑ l²i! Các hÕ ðang mang trong mình nhi®m vø v§n chuy¬n, thß½ng nhân ta không th¬ cung c¤p d¸ch vø cho các hÕ." )
		return
	end
	...
	if arg == 1000 then		--Tr· v«ÃÅÅÉ
		if id < 0 or id >= 9 then
			x001100_MsgBox( sceneId, selfId, targetId, "  Các hÕ chßa gia nh§p môn phái nào." )
		else
	...
	if arg == 1016 then		--Thúc Hà C± Tr¤n
			-- add by zchw
