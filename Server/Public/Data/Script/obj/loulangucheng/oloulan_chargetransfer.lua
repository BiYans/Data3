...
function x400959_UpdateEventList( sceneId, selfId, targetId )
	BeginEvent(sceneId);
		...
		AddNumText(sceneId, x400959_g_ScriptId, "#GThành th¸ - Phøng Minh Tr¤n", 9, 7);
	EndEvent()
...
function x400959_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if id >=1 and id <= 7 then
		BeginEvent(sceneId)
			...
			elseif id == 7 then
				AddNumText(sceneId, x400959_g_ScriptId, "Duy®t", 0, 71);
			end
	...
	elseif id == 100 then
		...
		if index == 7 then
			if GetLevel(sceneId, selfId) < 85 then
				BeginEvent(sceneId)
					AddText(sceneId, "  C¤p ğµ chßa ğü 85 không th¬ d¸ch chuy¬n")
				EndEvent()
				DispatchEventList(sceneId, selfId, targetId)
				return
			end
			...
			if LuaFnGetDRideFlag(sceneId, selfId) == 1 then
				...
				if objId ~= -1 and GetLevel(sceneId, objId) < 85 then
					BeginEvent(sceneId)
						AddText(sceneId, "BÕn hæu ng°i cßŞi chung không ğü c¤p 85, không th¬ d¸ch chuy¬n. ")
					EndEvent()
					DispatchEventList(sceneId, selfId, targetId)
					return					
				end
