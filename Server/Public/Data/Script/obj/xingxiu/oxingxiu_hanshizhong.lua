...
function x016001_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetNumText()==3	then
		...
		elseif GetLevel( sceneId, selfId ) < 10 then
			...
			elseif x016001_g_MenPai==9	then
				...
				-- °ÑÏà¹ØtoÕ ðµ ÐÄ·¨ÉèÖÃÎª10c¤p±ð  31,34,35
				...
				x016001_MsgBox( sceneId, selfId, "L¤y ðßþc 20 t¤m Chiêu t§p l®nh cüa môn phái" )

				if TryRecieveItem( sceneId, selfId, 10124005, 1 ) >= 0 then
					str		= "#YCác hÕ ðã nh§n ðßþc #W"..GetItemName( sceneId, 10124005 ).."."
	...
	elseif	GetNumText() == 2 then
		...
		if x016001_g_MenPai == 5 then
			...
			if RecipeFlag >= 1 then
				BeginEvent(sceneId)
						AddText(sceneId,"Các hÕ ðã h÷c làm l®nh tri®u t§p môn phái.")
				EndEvent(sceneId)
				DispatchEventList(sceneId,selfId,targetId)
				return 0;
			end
			...
			BeginEvent(sceneId)
				AddText(sceneId, "Ta chï truy«n thø cho ð® tØ cüa b±n phái")
			EndEvent(sceneId)
			DispatchEventList(sceneId,selfId,targetId)
		end
	
	elseif	GetNumText() == 999 then
		-- ¼ì²âÍæ¼ÒÐúng²»ÐúngÑ§»áÁË±ØÒªtoÕ ðµ ¼¼ÄÜ²¢´ïµ½±ØÒªµÈc¤p
		local	lev	= QueryHumanAbilityLevel( sceneId, selfId, 39 )
		if lev < 2 then
			BeginEvent(sceneId)
	...
	if GetNumText()==6 then
		...
		EndEvent(sceneId)
		DispatchEventList(sceneId, selfId, targetId)
		CallScriptFunction( SCENE_SCRIPT_ID, "AskTheWay", sceneId, selfId, sceneId, 87, 70, "Ê©È«" )
		return
	end
end
