...
function x335808_OnConditionCheck( sceneId, selfId )
	...
	if targetId < 0 or targetId == selfId then
		x335808_MsgBox(sceneId, selfId, "Chém yêu kiªm chï có th¬ ğ¯i niên thú sØ døng.");	
		return 0;
	end	
	--Ä¿±ê¶ÔÏóÀàĞÍ
	local objType = GetCharacterType( sceneId, targetId )
	if(objType ~= 2)then	--¶ÔÏó²»Ğúng¹ÖÎï
		x335808_MsgBox( sceneId, selfId, "Chém yêu kiªm chï có th¬ ğ¯i niên thú sØ døng.")	
		return 0;
	else
...
function x335808_OnActivateOnce( sceneId, selfId )
	...
	local extraExp = 1;
	if LuaFnHaveImpactOfSpecificDataIndex(sceneId, selfId, 62) == 1 then --Ğ¡Linhµ¤
		extraExp = 1.5;
	end
	...
	if extraExp > 1 then
		BeginEvent(sceneId)
			local msg = format("#{NSJYTS_1} nh§n ğßşc ph¥n thß·ng %d kinh nghi®m.", x335808_g_prize_exp[mstLevel] * (extraExp-1))
			AddText(sceneId, msg)
		EndEvent()
