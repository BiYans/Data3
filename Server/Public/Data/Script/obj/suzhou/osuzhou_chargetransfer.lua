...
x400957_g_transfer_target =
{
	...
	[7] = {x = 158, z = 121, scene_num = 480}, 	--Trß¶ng BÕch S½n
}
...
function x400957_UpdateEventList( sceneId, selfId, targetId )
	BeginEvent(sceneId);
		...
		AddNumText(sceneId, x400957_g_ScriptId, "#GThành th¸ - Phøng Minh Tr¤n", 9, 7);
	EndEvent()
...
function x400957_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if id >=1 and id <= 7 then
		BeginEvent(sceneId)
			...
			elseif id == 7 then
				AddNumText(sceneId, x400957_g_ScriptId, "Duy®t", 0, 71);
			end
			AddNumText(sceneId, x400957_g_ScriptId, "Hüy", 0, 100);
		EndEvent()
		DispatchEventList(sceneId, selfId, targetId);
	elseif id == 100 then
		...
		if index == 7 then
			if GetLevel(sceneId, selfId) < 85 then
				BeginEvent(sceneId)
					AddText(sceneId, "  C¤p ðµ chßa ðü 85 không th¬ d¸ch chuy¬n")
				EndEvent()
				DispatchEventList(sceneId, selfId, targetId)
				return
			end
			...
			if LuaFnGetDRideFlag(sceneId, selfId) == 1 then
				local objId = LuaFnGetDRideTargetID(sceneId, selfId);
				if objId ~= -1 and GetLevel(sceneId, objId) < 85 then
					BeginEvent(sceneId)
						AddText(sceneId, "BÕn hæu ng°i cßÞi chung không ðü c¤p 85, không th¬ d¸ch chuy¬n. ")
					EndEvent()
					DispatchEventList(sceneId, selfId, targetId)
					return					
				end
