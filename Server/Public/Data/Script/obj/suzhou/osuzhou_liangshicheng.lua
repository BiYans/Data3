...
function x001067_UpdateEventList( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		AddNumText( sceneId, x001067_g_ScriptId, "Ma Tôn H°n Vû ðúc Trùng Lâu", 6, 9999 )
...
function x001067_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if nNumText == 9999 then
		BeginEvent(sceneId)
			AddRadioItemBonus( sceneId, 10310236, 1 )
			AddRadioItemBonus( sceneId, 10310239, 1 )
			AddRadioItemBonus( sceneId, 10310238, 1 )
			AddRadioItemBonus( sceneId, 10310242, 1 )
			AddRadioItemBonus( sceneId, 10310235, 1 )
		EndEvent( )
...
function x001067_OnMissionSubmit( sceneId, selfId, targetId, missionScriptId, selectRadioId )
	...
	if selectRadioId >= 10310231 and selectRadioId <= 10310242 then
		--if GetName(sceneId, selfId) == "GameMaster" then
		--	local PosNew = TryRecieveItem(sceneId,selfId,selectRadioId ,1)
		--	local equipinfo = GetBagItemTransfer( sceneId, selfId, PosNew )
		--	local playername = GetName(sceneId, selfId)
		--	local strText = format("#{_INFOUSR%s} #Ylâu nay thu th§p #WMa Tôn H°n Vû #Ythì nay ðã ðúc thành cñc ph¦m #G#{_INFOMSG%s}.", playername,equipinfo)
		--	BroadMsgByChatPipe(sceneId,selfId, strText, 4)
		--end
		BeginEvent(sceneId)
			strText = "Ngß½i vçn chßa tìm ðßþc Ma Tôn H°n Vû?"
