--********************--
x741311_g_scriptId = 741311
x741311_g_wuhunIdList = {
...
--*        On Default Event        *--
--**********************************--
function x741311_OnDefaultEvent(sceneId,selfId,bagIndex)
...
--**********************************--
function x741311_IsSkillLikeScript(sceneId,selfId)
...
--*         Cancel Impacts         *--
--**********************************--
function x741311_CancelImpacts(sceneId,selfId)
...
--*       On Condition Check       *--
--**********************************--
function x741311_OnConditionCheck(sceneId,selfId)
	...
	local isWuhun = 0
	for i, wuhunId in x741311_g_wuhunIdList do
	...
	if isWuhun == 1 then
		...
		local VoHon_Level = CallScriptFunction(741302,"GetWuhunAuthor",sceneId,selfId,0,5,7)
...
--*           On Deplete           *--
--**********************************--
function x741311_OnDeplete(sceneId,selfId)
...
--*        On Activate Once        *--
--**********************************--
function x741311_OnActivateOnce(sceneId,selfId)
	--********************--
	local WHExpLevel = {}
		WHExpLevel[1] = {1500}
		WHExpLevel[2] = {1510}
	...
	local VoHon_Level = CallScriptFunction(741302,"GetWuhunAuthor",sceneId,selfId,0,5,7)
	local VoHon_Exp = CallScriptFunction(741302,"GetWuhunAuthor",sceneId,selfId,0,8,12)
	...
	CallScriptFunction(741302,"SetWuhunAuthor",sceneId,selfId,0,5,VoHon_Level..VoHon_Exp)
	x741311_Noitice(sceneId,selfId,"Võ H°n nuôi dßÞng thành công!")
...
--*        On Default Event        *--
--**********************************--
function x741311_OnActivateEachTick(sceneId,selfId)
...
function x741311_Noitice(sceneId,selfId,str)
	BeginEvent(sceneId)
		AddText(sceneId,str)
	EndEvent(sceneId)
	DispatchMissionTips(sceneId,selfId)
