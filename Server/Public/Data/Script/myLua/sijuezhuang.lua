...
x002939_g_SceneData_1 = 8   -- 记录副本刷怪到第几批
...
--x002939_g_SceneData_7 = 13	-- 玩家等级

x002939_g_SD_Monster_1 	 = 14			-- 暂未使用
x002939_g_SD_Monster_1_T = 15			-- 暂未使用
...
--副本名称
x002939_g_CopySceneName = "四绝庄"
x002939_g_CopySceneType = FUBEN_DAZHAN_SJZ	--副本类型，定义在ScriptGlobal.lua里面
x002939_g_CopySceneMap = "sijuezhuang.nav"
x002939_g_LimitMembers = 1				--可以进副本的最小队伍人数
x002939_g_TickTime = 1						--回调脚本的时钟时间（单位：秒/次）
x002939_g_DayTime = 3						--一天内可以进入副本的次数
x002939_g_LimitTotalHoldTime = 360--副本可以存活的时间（单位：次数）,如果此时间到了，则任务将会失败
x002939_g_LimitTimeSuccess = 500	--副本时间限制（单位：次数），如果此时间到了，任务完成
x002939_g_CloseTick = 3						--副本关闭前倒计时（单位：次数）
x002939_g_NoUserTime = 300				--副本中没有人后可以继续保存的时间（单位：秒）
x002939_g_DeadTrans = 0						--死亡转移模式，0：死亡后还可以继续在副本，1：死亡后被强制移出副本
x002939_g_Fuben_X = 100						--进入副本的位置X
x002939_g_Fuben_Z = 111						--进入副本的位置Z
x002939_g_Back_X = 197							--源场景位置X
x002939_g_Back_Z = 214						--源场景位置Z
x002939_g_Back_SceneId = 1				--源场景Id
...
function x002939_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if bOk == 0  then
		...
		BeginEvent(sceneId)
			AddText(sceneId,"#B挑战四绝庄");
			AddText(sceneId,"  你的队伍中" .. szAllName .. "今天已经参加过3次挑战四绝庄战役。");
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
		return
...
function x002939_OnEnumerate( sceneId, selfId, targetId )
	-- 最好在这里做一个名字判定
	if GetName(sceneId, targetId) == x002939_g_Name  then
		AddNumText( sceneId, x002939_g_ScriptId, "挑战四绝庄",10 ,-1  )
		AddNumText( sceneId, x002939_g_ScriptId, "关于挑战四绝庄",8 ,1  )
	end
end
...
function x002939_MakeCopyScene( sceneId, selfId )
	...
	LuaFnSetSceneLoad_Map(sceneId, "sijuezhuang.nav"); --地图是必须选取的，而且必须在Config/SceneInfo.ini里配置好
	...
	LuaFnSetCopySceneData_Param(sceneId, 0, x002939_g_CopySceneType);--设置副本数据，这里将0号索引的数据设置为999，用于表示副本号999(数字自定义)
	LuaFnSetCopySceneData_Param(sceneId, 1, x002939_g_ScriptId);--将1号数据设置为副本场景事件脚本号
