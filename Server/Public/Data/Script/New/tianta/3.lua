...
function x100013_OnDefaultEvent( sceneId, selfId, targetId )
		...
		if lev < 99 then
			...
			BeginEvent(sceneId)
		   		...
		   		AddText( sceneId, "  #G注意#Y:#cFF0000通天塔三层：#G开启时间为#cFF0000[19：45]。" )
		   		AddNumText( sceneId, x100013_g_ScriptId, "#e0ba4b6传送通天塔三层", 6, 30 )
			EndEvent(sceneId)
			DispatchEventList(sceneId,selfId,targetId)
		end
end
...
function x100013_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetNumText() == 30 then
       ...
       local nQuarter = mod(GetQuarterTime(),100);
            ...
            BeginEvent(sceneId)
		AddText(sceneId,"通天塔三层开启时间为(19:45），暂时无法传送!" )
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
		return 0
	end

     CallScriptFunction( (400900), "TransferFunc", sceneId, selfId, 547, 258, 314,10 )--传送
    end
end
