...
function x893066_OnDie( sceneId, selfId, killerId )
	...
	if playerName ~= nil then
		str = format("#cff99cc Khi #{_INFOUSR%s}#cff99cc dçn d¡t ðµi ngû ðánh bÕi #Y[T¥n V§n]#W#cff99cc · TÑ Tuy®t Trang, vô tình kích hoÕt c½ quan, báu v§t trong trang ð«u ¦n gi¤u trong ðó!",playerName); --ÇØÔÏ
...
function x893066_UseSkillG( sceneId, selfId )
	...
	CallScriptFunction((200060), "Paopao",sceneId, "Phan Tinh Tinh", "TÑ Tuy®t Trang", "T¯c ðµ cüa T¥n V§n càng lúc càng nhanh, xem ra ä s¡p liên tøc sØ døng H°i Âm Phiên r°i, m÷i ngß¶i hãy c¦n th§n!" )
	return 1
...
function x893066_OnImpactFadeOut( sceneId, selfId, impactId )
	...
	if impactId == x893066_BuffID_F1 then
		...
		CallScriptFunction((200060), "Paopao",sceneId, "Phan Tinh Tinh", "TÑ Tuy®t Trang", ""..GetName( sceneId, selfId ).." #HCác hÕ ðã trúng Chúc Änh Dao H°ng, mau tránh xa ð°ng ðµi cüa mình, nªu không s¨ làm hÕi ðªn h÷!" )
