--É±¹ÖµôÂäÈÎÎñ(ËùÓÐÕâmµt ÀàÐÍ¶¼Ê¹ÓÃCái này LUA,ÐÞ¸ÄÇë×¢ÒâÍ¨ÓÃÐÔ)
...
--**********************************
-- »ñÈ¡¶¯Ì¬Møc tiêu nhi®m vø NPC toÕ ðµ ´æ´¢Î»ÖÃ
-- Èç¹ûÐúng¾²Ì¬Møc tiêu nhi®m vø NPC,ÔòTr· v« -1
--**********************************
...
--**********************************
-- »ñÈ¡¶¯Ì¬Møc tiêu nhi®m vø NPC toÕ ðµ  NPCID
-- Èç¹ûÐúng¾²Ì¬Møc tiêu nhi®m vø NPC,ÔòTr· v« -1
--**********************************
...
--**********************************
-- »ñÈ¡¶¯Ì¬ÈÎÎñÎïÆ·toÕ ðµ  ItemIndex
-- Èç¹ûÐúng¾²Ì¬ÈÎÎñÎïÆ·,ÔòTr· v« -1
--**********************************
...
--ÈÎÎñÈë¿Úº¯Êý
--**********************************
function x006667_OnDefaultEvent( sceneId, selfId, targetId, missionIndex )	-- ði¬m»÷¸ÃÈÎÎñºóÖ´ÐÐ´Ë½Å±¾
	local missionId = TGetMissionIdByIndex( missionIndex )
...
function x006667_OnAccept( sceneId, selfId, targetId, missionIndex )
	...
	-- ÔÙ´Î¼ì²âÍæ¼ÒÐúng²»ÐúngÄÜ¹»Tiªp thøCái này ÈÎÎñ
	...
	SetMissionByIndex( sceneId, selfId, misIndex, x006667_g_IsMissionOkFail, 0 )	--¸ù¾ÝÐòÁÐºÅ°ÑÈÎÎñ±äÁ¿toÕ ðµ µÚmµt Î»ÖÃ0	µÚmµt Î»ÐúngÍê³É/th¤t bÕiÇé¿ö
...
function x006667_CheckSubmit( sceneId, selfId, missionIndex )
	...
	-- ¿´ ði¬mµ½toÕ ðµ ÎïÆ·Ðúng²»ÐúngÈÎÎñC¥n toÕ ðµ 
	local _, _, dynamicFlag = TGetLootItemInfo( missionIndex )
	-- PrintStr("dynamicFlag ="..dynamicFlag)
	if dynamicFlag ~= 2 then
		local a = { { name = "", flag = 0, item = 0, pro = 0, ct = 0 },
		...
		for i = 1, killDataCount do
			--  ðÕt ðßþcµ±Ç°Íæ¼ÒÓµÓÐCái này ÎïÆ·toÕ ðµ ÎïÆ·Êý
...
function x006667_OnSubmit( sceneId, selfId, targetId, selectRadioId, missionIndex )
	...
	if x006667_CheckSubmit( sceneId, selfId, missionIndex ) > 0 then
		...
		if ret <= 0 then
			x006667_NotifyFailTips( sceneId, selfId, "Không th¬ Hoàn t¤t nhi®m vø" )
			return
		end
		...
		if dynamicFlag ~= 2 then		
			...
			local LogInfo	= format( "[XunWu]New..Submit:( sceneId=%d, GUID=%0X ), missionIndex=%d, selectRadioId=%d",
													...
													selectRadioId )
			MissionLog(sceneId, LogInfo)
			local ItemType = GetItemClass( selectRadioId )											--  ðÕt ðßþcÌá½»ÎïÆ·toÕ ðµ Class
			...
			while (ItemPos ~= -1)do
				...
				startpos = ItemPos+1																							--´ÓÏÂmµt cáiÎ»ÖÃ¿ªÊ¼ÕÒ
				
				if (LuaFnIsItemAvailable( sceneId, selfId, ItemPos ) ~= 0) then		--Ðúng·ñËø¶¨
					if ItemType == 1 then																						-- Ðúng·ñÊôÓÚ¿ÉÒÔ¼ø¶¨toÕ ðµ ÎïÆ·
						if LuaFnIsJudgeApt(sceneId, selfId, ItemPos) == 1 then				--Ðúng·ñ¼ø¶¨¹ý
							break																												--¼ø¶¨¹ý²ÅÄÜ¿Û³ý
						end
					else
			...
			local isdelitem  = 0
			if ItemType == 1 then
				isdelitem =	EraseItem(sceneId, selfId, ItemPos)													--C¥n ¼ø¶¨toÕ ðµ °´Î»ÖÃÉ¾³ý
			else	
				isdelitem =	LuaFnDelAvailableItem( sceneId, selfId, selectRadioId, 1 )	--²»C¥n ¼ø¶¨toÕ ðµ °´ÎïÆ·É¾³ý(ÕâÀïÈç¹ûÖ±½ÓEraseItem¶ÑµþÎïÆ·»á³öÎÊÌâ)
			end
...
--É±ËÀ¹ÖÎï»òÍæ¼Ò
--**********************************
function x006667_OnKillObject( sceneId, selfId, objdataId, objId, missionIndex )	--²ÎÊýÒâË¼: ³¡¾°ºÅ¡¢Íæ¼ÒobjId¡¢¹ÖÎï±íÎ»ÖÃºÅ¡¢¹ÖÎïobjId
	local missionId = TGetMissionIdByIndex( missionIndex )
	local missionName = TGetMissionName( missionIndex )
	...
	for i = 1, killDataCount do
		if monsterName == a[i].name then
			local num = GetMonsterOwnerCount( sceneId, objId )				--È¡ ði¬mCái này ¹ÖÎïËÀºóÓµÓÐ·ÖÅäÈ¨toÕ ðµ ÈËÊý
			for j = 0, num - 1 do
				local humanObjId = GetMonsterOwnerID( sceneId, objId, j )	--È¡ ði¬mÓµÓÐ·ÖÅäÈ¨toÕ ðµ ÈËtoÕ ðµ objId
				if IsHaveMission( sceneId, humanObjId, missionId ) > 0 then	--Èç¹ûCái này ÈËÓµÓÐÈÎÎñ
					if GetItemCount( sceneId, humanObjId, a[i].item ) < a[i].ct then
						-- C¥n ¼ÆËã¸ÅÂÊ
						local nPro = random( 100 )
						if nPro <= a[i].pro then
							AddMonsterDropItem( sceneId, objId, humanObjId, a[i].item )	--¸øCái này ÈËÈÎÎñµÀ¾ß(µÀ¾ß»á³öÏÖTÕi Ê¬Ìå°üÀï)
						end
					end
				end
			end
...
function x006667_OnItemChanged( sceneId, selfId, itemdataId, missionIndex )
	...
	for i = 1, killDataCount do
		...
		if itemdataId == a[i].item then
			...
			-- Ö»ÓÐTÕi ÎïÆ·cáiÊýÐ¡ÓÚC¥n »òÕßµÈÓÚC¥n toÕ ðµ Ê±ºò²ÅÏÔÊ¾
			if a[i].ct >= nItemNum then
	...
	for i=1, 3 do
		...
		if index < 100 then	
			...
			if submitcnt > 0 then
				...
				local ItemType = GetItemClass( ItemID )				--  ðÕt ðßþcÌá½»ÎïÆ·toÕ ðµ Class
				...
				x006667_NotifyFailTips( sceneId, selfId, "Không th¬ Hoàn t¤t nhi®m vø" )					
				BeginEvent(sceneId)
					AddText(sceneId, "Này cûng không gi¯ng nhß là ta c¥n  v§t ph¦m.")
					EndEvent( )
				DispatchEventList( sceneId, selfId, targetId )
