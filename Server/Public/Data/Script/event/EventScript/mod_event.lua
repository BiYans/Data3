-- mod_event
-- ÈÎÎñ±í¸ñÖÐtoÕ ðµ mµt Ð©Í¨ÓÃÊý¾Ý´¦Àíº¯Êý
...
-- 30~50c¤pÍæ¼ÒÁ÷Ê§ÈÎÎñ_Nhi®m vø sß môn
...
function x006672_DoEnumerate( sceneId, selfId, targetId, missionIndex )
	...
	-- Èç¹ûÍæ¼ÒÍê³É¹ýCái này ÈÎÎñ²¢ÇÒ¸ÃÈÎÎñ²»ÐúngÑ­»·ÈÎÎñ
...
--**********************************
-- È¡ ði¬mmµt cái¶¯Ì¬toÕ ðµ ¶Ô»°¸ñÊ½»¯×Ö·û´®
-- missionIndex: ÈÎÎñTÕi ÈÎÎñ±íÖÐtoÕ ðµ ±àºÅ
-- duologueContent: ¶Ô»°ÄÚÈÝ,Èç¹ûÐúng¹Ì¶¨¶Ô»°,ÔòÖ±½ÓTr· v«
--**********************************
function x006672_GetRandomDuologue( sceneId, missionIndex, duologueContent )
	
	-- Ëæ»ú¶Ô»°C¥n ÏÈÌôÑ¡³ömµt ¾äÀ´
	local duologueList = {}
	...
	-- ÐÂÔötoÕ ðµ ½Ó¿Ú¶ÁÐÂÔötoÕ ðµ ±í¸ñ,È¡»ØÄ³¾ä¶Ô»°
...
--**********************************
-- ¸ñÊ½»¯mµt cái¶¯Ì¬toÕ ðµ ¶Ô»°¸ñÊ½»¯×Ö·û´®
-- ¸ñÊ½»¯ÖÐtoÕ ðµ Ä£Ê½
--	%R: Íæ¼ÒtoÕ ðµ ³ÆÎ½( các hÕ,ÏÀÅ®)
--	%n: NPC toÕ ðµ Tên
--	%l: NPC toÕ ðµ Î»ÖÃÒÔ¼°×ø±ê
--	%g: NPC toÕ ðµ ÐÔ±ð(Ëû,Ëý)
--	%i: ÈÎÎñÎïÆ·toÕ ðµ Tên
--**********************************
...
function x006672_DisplayBonus( sceneId, missionIndex, selfId )
	...
	if (missionIndex >= 1010243 and missionIndex <= 1010250) or
		...
		-- Ê¹ÓÃÍæ¼Ò×Ô¼ºtoÕ ðµ µÈc¤pÀ´¼ÆËã ði¬mµ½toÕ ðµ ½±Àø
...
function x006672_RewardRelationShip( sceneId, selfId, missionIndex, targetId )
	...
	-- ¼ÇÂ¼ºÍCái này Npc½ñÌìHoàn t¤t nhi®m vøtoÕ ðµ ´ÎÊý
	if nLimitiIndex >= MD_JQXH_MUWANQING_LIMITI and 
		...
		nDayTime = 	nTime											--ÉÏmµt ´Î½»ÈÎÎñtoÕ ðµ Ê±¼ä(ÌìÊý)
	...
	if value > x006672_g_MaxRelationship then
		...
		-- Èç¹ûÍæ¼ÒºÍNpc´ïµ½mµt cáiÐÂtoÕ ðµ ¹ØÏµ,·¢¹«¸æ
		if value == 1000  then
			BeginEvent(sceneId)
	...
	-- hongyu ,×Å¼±ÁË,TÕi ÕâÀïÏÈ+Kinh nghi®mºÍ½ðÇ®½±Àø
	-- ½±ÀøtoÕ ðµ ½ðÇ®ºÍKinh nghi®mÖµ,Ö»ºÍ×Ô¼ºtoÕ ðµ µÈc¤pÓÐ¹Ø
	-- µÈc¤p±ØÐëÐúng´óÓÚµÈÓÚ20,Ð¡ÓÚµÈÓÚPlayerMaxLevel
	-- ¹«Ê½  Kinh nghi®m= µÈc¤p*80 - 326
	-- 			 ½ðÇ®= µÈc¤p*18 - 101
