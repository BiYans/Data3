...
--¸±±¾Ãû³Æ
x402105_g_CopySceneName = "HoÕt ðµng Tr¤n áp bÕo t¦u"

x402105_g_CopySceneType = FUBEN_CATCH_PET	--¸±±¾ÀàÐÍ,¶¨ÒåTÕi ScriptGlobal.luaÀïÃæ

x402105_g_CopySceneMap = "zhenshoulan.nav"
x402105_g_Exit = "zhenshoulan.ini"
x402105_g_LimitMembers = 3				--¿ÉÒÔ½ø¸±±¾toÕ ðµ ×îÐ¡¶ÓÎéÈËÊý
x402105_g_TickTime = 1						--»Øµ÷½Å±¾toÕ ðµ Ê±ÖÓÊ±¼ä(µ¥Î»:  giây/´Î)
x402105_g_LimitTotalHoldTime = 360--¸±±¾¿ÉÒÔ´æ»îtoÕ ðµ Ê±¼ä(µ¥Î»: ´ÎÊý),Èç¹û´ËÊ±¼äµ½ÁË,ÔòÈÎÎñ½«»áth¤t bÕi
x402105_g_LimitTimeSuccess = 500	--¸±±¾Ê±¼äÏÞÖÆ(µ¥Î»: ´ÎÊý),Èç¹û´ËÊ±¼äµ½ÁË,ÈÎÎñÍê³É
x402105_g_CloseTick = 3						--¸±±¾¹Ø±ÕÇ°µ¹¼ÆÊ±(µ¥Î»: ´ÎÊý)
x402105_g_NoUserTime = 300				--¸±±¾ÖÐÃ»ÓÐÈËºó¿ÉÒÔ¼ÌÐø±£´ætoÕ ðµ Ê±¼ä(µ¥Î»:  giây)
x402105_g_DeadTrans = 0						--ËÀÍö×ªÒÆÄ£Ê½,0: ËÀÍöºó»¹¿ÉÒÔ¼ÌÐøTÕi ¸±±¾,1: ËÀÍöºó±»Ç¿ÖÆÒÆ³ö¸±±¾
x402105_g_Fuben_X = 36						--½øÈë¸±±¾toÕ ðµ Î»ÖÃX
x402105_g_Fuben_Z = 97						--½øÈë¸±±¾toÕ ðµ Î»ÖÃZ
...
function x402105_OnDefaultEvent( sceneId, selfId, targetId )
	...
	-- Ö»C¥n ¶Ó³¤ÓÐÈÎÎñÎïÆ·¾Í¿ÉÒÔ,¼ì²â¶Ó³¤ÉíÉÏÐúng²»ÐúngÓÐCái này ÎïÆ·
	if LuaFnGetAvailableItemCount(sceneId,selfId,x402105_g_Item) < 1  then
		BeginEvent(sceneId)
			AddText(sceneId,"#BÐu±i b¡t Dã Trß Vß½ng");
			AddText(sceneId,"  Nªu mu¯n tìm ðªn Dã Trß Vß½ng, chúng ta có cûng ðü  manh m¯i m¾i ðßþc. #R(C¥n mang theo ðÕo cø #GTung tích Dã Trß Vß½ng.#R)");
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
		return
	end
...
function x402105_OnEnumerate( sceneId, selfId, targetId )
	...
	AddNumText( sceneId, x402105_g_ScriptId, "Ðu±i b¡t Dã Trß Vß½ng",10 ,-1  )
...
function x402105_MakeCopyScene( sceneId, selfId, targetId )
	...
	LuaFnSetSceneLoad_Map(sceneId, "zhenshoulan.nav"); --µØÍ¼Ðúng±ØÐëÑ¡È¡toÕ ðµ ,¶øÇÒ±ØÐëTÕi Config/SceneInfo.iniÀïÅäÖÃºÃ
	...
	LuaFnSetCopySceneData_Param(sceneId, 0, x402105_g_CopySceneType);--ÉèÖÃ¸±±¾Êý¾Ý,ÕâÀï½«0ºÅË÷ÒýtoÕ ðµ Êý¾ÝÉèÖÃÎª999,ÓÃÓÚ±íÊ¾¸±±¾ºÅ999(Êý×Ö×Ô¶¨Òå)
	LuaFnSetCopySceneData_Param(sceneId, 1, x402105_g_ScriptId);--½«1ºÅÊý¾ÝÉèÖÃÎª¸±±¾³¡¾°ÊÂ¼þ½Å±¾ºÅ
	...
	-- Ô­À´toÕ ðµ Ä£Ê½ ÕætoÕ ðµ ºÜÆæ¹ÖÃ»ÓÐ -1 ¼ÇÂ¼ÒªTiªp cÑu toÕ ðµ ¹ÖÎïtoÕ ðµ »ù´¡Id
	...
	if bRetSceneID>0 then			
		-- Èç¹ûÍæ¼ÒÐúng¶Ó³¤,¿Û³ýÈÎÎñµÀ¾ß							modify by WTT		20090225
...
function x402105_OnDie( sceneId, objId, killerId )
	...
	if GetName(sceneId, objId) == "Tiêm Nha Dã Trß"  then
		...
		x402105_TipAllHuman(sceneId, "Ðã giªt chªt Tiêm Nha Dã Trß " .. tostring(nKillCount+1) .. "/30")
	end
end
--**********************************
--½øÈëÇøÓòÊÂ¼þ
...
function x402105_OnCopySceneTimer( sceneId, nowTime )
	...
	if nStep == 2  then
		...
		if nHaveMonster == 0  then
			...
			-- ·¢ÊÀ½ç¹«¸æ
			if nType == 0  then
				str = format("#{_INFOUSR%s}#P dçn d¡t ðµi ngû tÕi #GNông trß¶ng Dã Trß #Ptiêu di®t Dã Trß Thü Lînh #{_BOSS20}#P.",szLeaderName)
			else
				str = format("#{_INFOUSR%s}#P dçn d¡t ðµi ngû tÕi #GNông trß¶ng Dã Trß #Pdi®t tr× ð¥u sö #{_BOSS21}#P, cu¯i cùnb bình ±n tr§n phong ba này.",szLeaderName)
			end
...
function x402105_CreateMonster_2(sceneId)
	...
	if nRand <= x402105_g_BossRand_2   then
	  ...
	  if nInilevel >= 11 then
		...
		str = format("#P— #{_INFOUSR%s}#P dçn d¡t ðµi ngû, · Thánh Thú S½n tr¤n áp ðàn Dã Trß th¡ng lþi, ðµt nhiên nhäy ra #{_BOSS21}.",szLeaderName)
		BroadMsgByChatPipe(sceneId, nLeaderId, str, 4)
