...
x311200_Stiletto_four_ID = {
														...
														10422016,10423024,
														--H° Khäi,2008.8.29.¾É100Ì×(Îå¼þÌ×)¼°ÐÂ96Ì×¿ª·Å¼«ÏÞ´ò¿×
														...
														--H° Khäi,2008.9.18.90c¤pÒÔÉÏ(º¬90)Éú»î¼¼ÄÜ²ú³ötoÕ ðµ ½äÖ¸,»¤·û,¼ç¿ª·Å¼«ÏÞ´ò¿×
														10215020,10222020,10223020,10222035,10222036,10223035,10223036,
														--H° Khäi,2008.11.11.90c¤pÒÔÉÏ(º¬90)ÊÖ¹¤×°±¸¿ª·Å¼«ÏÞ´ò¿×(Ð¬,Ñü´ø,»¤Íó,ÊÖÌ×,Í·¿ø,ÎäÆ÷,»¤¼×,ÏîÁ´)
														...
														--zchw,2008-11-17  TT: 41140 90ÃÅÅÉÌ×,92c¤pÉñÆ÷¿ª·ÅµÚËÄ¿×													
...
function x311200_OnStiletto( sceneId, selfId, idBagPos, idBagPosStuff )
	...
	elseif ret == -4 then
		x311200_NotifyTip( sceneId, selfId, "Không th¬ khoan ðßþc næa" ) --modi:lby2008-5-21µ±Íæ¼ÒÆóÍ¼´òµÚËÄcái¿ÕtoÕ ðµ Ê±ºò±¨´í
...
--add:lby20080523TÕi Â¥ÄÏ´òµÚËÄcái¿Õ
function x311200_OnStiletto_Four( sceneId, selfId, idBagPos, idBagPosStuff, type )
	...
	if (IsItemDark(sceneId, selfId, taozhuangid) == 1) then
	 ...
	 if type ~= 1 and type ~= 2 then
	    x311200_NotifyTip( sceneId, selfId, "Sai l¥m nghiêm tr÷ng, tiêu hao loÕi hình." )
			return
	 end
	...
	elseif ret == 0 then --zchw
		x311200_NotifyTip( sceneId, selfId, "Tr× ti«n th¤t bÕi" );
		return
	end
	...
	if ret == -1 then
		x311200_NotifyTip( sceneId, selfId, "Ðøc l² trang b¸ th¤t bÕi" )
	elseif ret == -2 then
		x311200_NotifyTip( sceneId, selfId, "Sai l¥m nghiêm tr÷ng" )
	elseif ret == -3 then
	...
	--Ð´ÐÐÎªÈÕÖ¾
	  ...
	  AuditSlotFour(sceneId, selfId, strLog)
		...
		x311200_NotifyTip( sceneId, selfId, "Ðøc l² trang b¸ thành công" )
		...
		if isequipbind == 0 and isstuffbind == 1 and type == 2 then
	 	...
	 	local str = ""
	 	if type == 1 then
			CostMoney(sceneId, selfId, 10000000)
