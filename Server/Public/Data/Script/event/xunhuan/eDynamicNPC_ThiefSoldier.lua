...
x050013_g_LimitMembers			= 3		--¿ÉÒÔ½ø¸±±¾toÕ ðµ ×îÐ¡¶ÓÎéÈËÊý
...
function x050013_OnEnumerate( sceneId, selfId, targetId )
	...
	local Fb1, Fb2, Fb3, Fb4 = CallScriptFunction(741307,"FubenChecker",sceneId,selfId,050013,0)
	...
	if GetTeamSize( sceneId, selfId ) < Fb1 then
		AddText( sceneId, "Mu¯n thØ thách ta chí ít cûng phäi t¾i "..Fb1.." ngß¶i. Có tí ngß¶i thª này, quá coi thß¶ng ta" )
		return
	end
	...
	notifyString = notifyString..") ðÆng c¤p không ðü"..Fb2..".";	
	if(namenum > 0) then
		BeginEvent( sceneId )
	...
	for i=0, NearCount-1 do
		...
		if( Level < Fb2 ) then
			BeginEvent( sceneId )
				AddText( sceneId, "Trong nhóm cüa ngß½i có ngß¶i có ðÆng c¤p không ðü c¤p "..Fb2.."!" )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
			return
		end
...
function x050013_OnAccept( sceneId, selfId )
	...
	local Fb1, Fb2, Fb3, Fb4 = CallScriptFunction(741307,"FubenMinMember",sceneId,selfId,050013,0)
  --´«ËÍÇ°,ÒªÖØÐÂÅÐ¶Ïmµt ´Î½øÈëÌõ¼þ Steven.Han 2006-12-27 13:53
	...
	if GetTeamSize( sceneId, selfId ) < Fb1 then
		x050013_NotifyTip( sceneId, selfId, "Nhóm không ðü "..Fb1.." ngß¶i." )
		return -1
	end
