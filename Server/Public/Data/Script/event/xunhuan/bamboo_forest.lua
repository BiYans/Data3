...
x050101_g_LimitMembers = 3					-- ¿ÉÒÔ½ø¸±±¾toÕ ğµ ×îĞ¡¶ÓÎéÈËÊı
...
x050101_g_TakeTimes = 5											-- Ã¿Ìì×î¶àLînh ´ÎÊı
--**********************************
-- ÈÎÎñÈë¿Úº¯Êı
--**********************************
...
function x050101_OnAccept( sceneId, selfId, targetId )
	if IsHaveMission( sceneId, selfId, x050101_g_MissionId ) == 0 then					-- Ã»ÓĞÈÎÎñ²Å¿ÉÒÔ×ßÕâÀï
		...
		local Fb1, Fb2, Fb3, Fb4 = CallScriptFunction(741307,"FubenChecker",sceneId,selfId,050101,0)
		if GetLevel( sceneId, selfId ) < Fb2 then
			x050101_NotifyFailBox( sceneId, selfId, targetId, "    ĞÆng c¤p cüa ngß½i quá th¤p, không th¬ chiªn th¡ng. ĞÕt c¤p "..Fb2.." hãy ğªn tìm ta" )
			return
		end
...
function x050101_AcceptEnterCopyScene( sceneId, selfId, targetId )
	if IsHaveMission( sceneId, selfId, x050101_g_MissionId ) > 0 then					-- ÓĞÈÎÎñ²Å¿ÉÒÔ×ßÕâÀï
		...
		if FbTime ~= 0 then
			x050101_NotifyFailBox( sceneId, selfId, targetId, "Còn "..FbTime.." phút næa máy chü s¨ tiªn hành bäo trì, các phø bän ğã ğóng." )
			return
		end
		...
		if nearMemberCount < Fb1 then
			x050101_NotifyFailBox( sceneId, selfId, targetId, "    Trong r×ng trúc g¤u dæ r¤t nhi«u ít nh¤t các ngß½i phäi có "..Fb1.." ngß¶i ğ°ng hành, ta m¾i yên tâm ğ¬ các ngß½i ği." )
			return
		end
...
function x050101_OnCopySceneReady( sceneId, destsceneId )
	...
	if validmembercount < Fb1 then
		x050101_NotifyFailTips( sceneId, leaderObjId, "    Trong r×ng trúc g¤u dæ r¤t nhi«u, trong nhóm c¥n có "..Fb1.." nhân v§t tr· lên." )
		return
	end
