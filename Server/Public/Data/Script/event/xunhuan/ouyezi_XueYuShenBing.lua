...
function x500503_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if IsHaveMission( sceneId, selfId, x500503_g_MissionId ) > 0 then				
		...
		BeginEvent(sceneId)
			...
			AddText( sceneId, "#r#GÐÆng c¤p nhi®m vø: "..shenqilvl )
		EndEvent( )
...
function x500503_GetKillCountByLevel(iItemLevel)
	...
	elseif (iItemLevel >= 50 and iItemLevel < 60) then
		iCount = 1500
	elseif (iItemLevel >= 60 and iItemLevel < 70) then
		iCount = 2000
	elseif (iItemLevel >= 70 and iItemLevel < 80) then
		iCount = 3000
	elseif (iItemLevel >= 80 and iItemLevel < 90) then
		iCount = 4000
	end
...
--É±ËÀ¹ÖÎï»òÍæ¼Ò
--**********************************
function x500503_OnKillObject( sceneId, selfId, objdataId ,objId)
		...
		if IsHaveMission(sceneId, humanObjId, x500503_g_MissionId) > 0 then
			...
			if GetMissionParam(sceneId, humanObjId, misIndex, 0) <=0  then
				...
				if ((PlayerLevel - MonsterLevel >= 0) and (PlayerLevel - MonsterLevel < 10))
					...
					BeginEvent(sceneId)
						local str = "Ðã giªt chªt quái v§t yêu c¥u: "..killedCount.."/"..demandKillCount	
						AddText(sceneId, str)
					EndEvent(sceneId)
