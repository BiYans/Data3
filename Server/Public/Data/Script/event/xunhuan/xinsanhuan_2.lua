...
-- ÈÎÎñºÅ
x050221_g_MissionId = 1257					--1256 »Æ½ðÖ®Á´--1257 Huy«n Ph§t Châu--1258 ÈÛÑÒÖ®µØ
...
--x050221_g_ContinueInfo = "    ÄãÃÇ×¼±¸ºÃÁË¾ÍÇëÇ°È¥ÖñÁÖÏûÃðºìÐÜÍõ£¡"					-- Î´Íê³ÉÈÎÎñµÄnpc¶Ô»°
x050221_g_SubmitInfo = "#{LLFB_80816_43}"										-- Ìá½»Ê±µÄ´ð¸´
x050221_g_MissionComplete = "#{LLFB_80902_2}"	--Íê³ÉÈÎÎñnpcËµµÄ»°

x050221_g_IsMissionOkFail = 0														-- 0 ºÅË÷Òý£ºµ±Ç°ÈÎÎñÊÇ·ñÍê³É(0Î´Íê³É£»1Íê³É£»2Ê§°Ü)
x050221_g_IsBossYaoWang = 1															-- 1 ºÅË÷Òý£ºÉ±bossH°ng Cúc Yêu Vß½ngµÄÊýÁ¿
x050221_g_IsFindGoods = 2																-- 2 ºÅË÷Òý£ºÊÇ·ñÕÒµ½Huy«n Ph§t Châu
x050221_g_Param_sceneid = 3															-- 3 ºÅË÷Òý£ºµ±Ç°ÈÎÎñÊý¾ÝµÄ3Î»ÖÃÉèÖÃÎª³¡¾°ID
...
x050221_g_heyuanxin = 40004460	--ºÎÔÃµÄÐÅ
x050221_g_xuanfouzhu = 40004454	--Huy«n Ph§t Châu
...
x050221_g_BossYaoWang = "H°ng Cúc Yêu Vß½ng"			-- bossH°ng Cúc Yêu Vß½ng
...
x050221_g_CopySceneType = FUBEN_XUANFOUZHU		-- ¸±±¾ÀàÐÍ£¬¶¨ÒåÔÚScriptGlobal.luaÀïÃæ
x050221_g_LimitMembers = 1					-- ¿ÉÒÔ½ø¸±±¾µÄ×îÐ¡¶ÓÎéÈËÊý
x050221_g_TickTime = 5						-- »Øµ÷½Å±¾µÄÊ±ÖÓÊ±¼ä£¨µ¥Î»£ºÃë/´Î£©
x050221_g_LimitTotalHoldTime = 360			-- ¸±±¾¿ÉÒÔ´æ»îµÄÊ±¼ä£¨µ¥Î»£º´ÎÊý£©,Èç¹û´ËÊ±¼äµ½ÁË£¬ÔòÈÎÎñ½«»áÊ§°Ü
x050221_g_CloseTick = 6						-- ¸±±¾¹Ø±ÕÇ°µ¹¼ÆÊ±£¨µ¥Î»£º´ÎÊý£©
x050221_g_NoUserTime = 30					-- ¸±±¾ÖÐÃ»ÓÐÈËºó¿ÉÒÔ¼ÌÐø±£´æµÄÊ±¼ä£¨µ¥Î»£ºÃë£©
...
x050221_g_TakeTimes = 5											-- Ã¿Ìì×î¶àÁìÈ¡´ÎÊý
--**********************************
-- ÈÎÎñÈë¿Úº¯Êý
--**********************************
function x050221_OnDefaultEvent( sceneId, selfId, targetId )	-- µã»÷¸ÃÈÎÎñºóÖ´ÐÐ´Ë½Å±¾
	if GetName( sceneId, targetId ) ~= x050221_g_Name then		-- ÅÐ¶Ï¸ÃnpcÊÇ·ñÊÇ¶ÔÓ¦ÈÎÎñµÄnpc
		return
	end
...
function x050221_OnAccept( sceneId, selfId, targetId )
	if LuaFnDelAvailableItem( sceneId, selfId, x050221_g_heyuanxin, 1 ) <= 0 then--ÐèÒª¿Û³ýºÎÔÃµÄÐÅ
		x050221_NotifyFailBox( sceneId, selfId, targetId, "#{LLFB_80816_21}" )
		return
	end
...
--	SetMissionEvent( sceneId, selfId, x050221_g_MissionId, 4 ) -- ×¢²á OnLockedTarget ÊÂ¼þ
	...
	--ÐèÒª¿Û³ýÒ»´ÎÒýÄ§Öé£¬ÉÏ´ÎÈÎÎñÊ§°Ü¿ÉÄÜÉíÉÏ»¹ÓÐÒýÄ§Öé´æÔÚ
...
function x050221_AcceptEnterCopyScene( sceneId, selfId, targetId )
	if IsHaveMission( sceneId, selfId, x050221_g_MissionId ) > 0 then					-- ÓÐÈÎÎñ²Å¿ÉÒÔ×ßÕâÀï
		...
		if FbTime ~= 0 then
			x050221_NotifyFailBox( sceneId, selfId, targetId, "Còn "..FbTime.." phút næa máy chü s¨ tiªn hành bäo trì, các phø bän ðã ðóng." )
			return
		end
		...
		local Fb1, Fb2, Fb3, Fb4 = CallScriptFunction(741307,"FubenChecker",sceneId,selfId,050221,0)
		
		if not nearMemberCount or nearMemberCount < Fb1 then
			x050221_NotifyFailBox( sceneId, selfId, targetId, "  T¯i thi¬u phäi có "..Fb1.." thành vi®n trong ðµi m¾i có th¬ yên tâm ðªn Ðµc Chß¾ng TrÕch. (Yêu c¥u ðÕt #Gc¤p "..Fb2.."#W, tâm pháp ðÕt ðªn #Gc¤p "..Fb3.."#W)" )
			return
		end
		...
		for	i = 0, nearMemberCount - 1 do
			...
			if x050221_CheckAllXinfaLevel(sceneId, memberID, Fb3) == 1 then
		...
		LuaFnSetSceneLoad_Map( sceneId, x050221_g_CopySceneMap )						-- µØÍ¼ÊÇ±ØÐëÑ¡È¡µÄ£¬¶øÇÒ±ØÐëÔÚConfig/SceneInfo.iniÀïÅäÖÃºÃ
		...
		LuaFnSetCopySceneData_Param( sceneId, 0, x050221_g_CopySceneType )				-- ÉèÖÃ¸±±¾Êý¾Ý£¬ÕâÀï½«0ºÅË÷ÒýµÄÊý¾ÝÉèÖÃÎª999£¬ÓÃÓÚ±íÊ¾¸±±¾ºÅ999(Êý×Ö×Ô¶¨Òå)
		...
		LuaFnSetCopySceneData_Param( sceneId, 14, 0 )							-- ÊÇ·ñÉ±ËÀPhá Di®m Tôn Giä
		LuaFnSetCopySceneData_Param( sceneId, 15, 0 )							-- ÊÇ·ñÒý³öH°ng Cúc Yêu Vß½ng
...
--É±ËÀ¹ÖÎï»òÍæ¼Ò
--**********************************
function x050221_OnKillObject( sceneId, selfId, objdataId, objId )						-- ²ÎÊýÒâË¼£º³¡¾°ºÅ¡¢Íæ¼ÒobjId¡¢¹ÖÎï±íÎ»ÖÃºÅ¡¢¹ÖÎïobjId
end
...
function x050221_OnAbandon( sceneId, selfId )
	...
	if sceneId == copyscene and fubentype == x050221_g_CopySceneType then											-- Èç¹ûÔÚ¸±±¾ÀïÉ¾³ýÈÎÎñ£¬ÔòÖ±½Ó´«ËÍ»Ø
...
--	local LevelGap = LuaFnGetCopySceneData_Param( sceneId, CopyScene_LevelGap )
	...
	elseif TickCount == x050221_g_LimitTotalHoldTime then									-- ¸±±¾×ÜÊ±¼äÏÞÖÆµ½ÁË
			...
			if LuaFnIsObjValid( sceneId, mems[i] ) == 1 and LuaFnIsCanDoScriptLogic( sceneId, mems[i] ) == 1 then
				...
				if IsHaveMission( sceneId, mems[i], x050221_g_MissionId ) > 0 then
					...
					if GetMissionParam( sceneId, mems[i], misIndex, x050221_g_IsMissionOkFail ) ~= 1 then
						SetMissionByIndex( sceneId, mems[i], misIndex, x050221_g_IsMissionOkFail, 2 )	-- Èç¹ûÃ»ÓÐÍê³É¾ÍÉèÖÃÎªÊ§°Ü
					end
				end
				x050221_Exit( sceneId, mems[i] )
			end
		...
		local isFireBoss = LuaFnGetCopySceneData_Param( sceneId, 14 )				-- ÊÇ·ñÉ±ËÀPhá Di®m Tôn Giä
		local isYaoWang = LuaFnGetCopySceneData_Param( sceneId, 15 )			-- ÊÇ·ñÒý³öH°ng Cúc Yêu Vß½ng
		local NotifyTime = LuaFnGetCopySceneData_Param( sceneId, 16 )			-- ÌáÐÑÊ±¼ä
		
		--Ã¿2 phútÌáÊ¾Ò»´Î£¬µ¥Êý¼ÆÊ±
		local isPreNotify = 0
		...
		--Ð¡ÓÚ15 phútµÄÊ±ºò£¬É±ËÀÁËPhá Di®m Tôn Giä£¬ÐèÒªÃ¿2·ÖÌáÐÑÒ»´Îboss³öÏÖÊ±¼ä
		local MonsterCheck = GetMonsterCount(sceneId)
		if MonsterCheck == 1 and isFireBoss == 1 and isYaoWang == 0 then
...
function x050221_CheckSubmit( sceneId, selfId )
	...
	local misIndex = GetMissionIndexByID( sceneId, selfId, x050221_g_MissionId )				-- µÃµ½ÈÎÎñµÄÐòÁÐºÅ
...
function x050221_OnSubmit( sceneId, selfId, targetId, selectRadioId )
	...
	if x050221_CheckSubmit( sceneId, selfId ) == 1 then
		...
		--ÐèÒª¿Û³ýÒ»´ÎÖÐ¼äÈÎÎñÎïÆ·ÒýÄ§Öé£¬Ã»ÓÐ¾Í²»¹ÜËü
		for i = 1, getn( x050221_g_YinMoZhuID ) do
			local num = LuaFnGetAvailableItemCount( sceneId, selfId, x050221_g_YinMoZhuID[i] )
			if num > 0 then
				LuaFnDelAvailableItem( sceneId, selfId, x050221_g_YinMoZhuID[i], num )
...
function x050221_GenerateMonster(sceneId, selfId, flag)
	...
	local ret = 0 --·µ»ØÖµ£¬1ÒýÄ§Öé»áÏûÊ§£¬0²»»áÏûÊ§
	if flag >= 1 and flag <= 5 then
		...
		if inpostype == flag then--µØµãÓÅÏÈ£¬ÊÇ·ñÉ±bossÆä´Î
			...
			for i = (inpostype-1)*10+1,inpostype*10  do--ÔÚÊ¹ÓÃµØÒý³ö10¸öÐ¡¹Ö£¬¶ø²»ÊÇÔÚÒýÄ§ÖéµÄÔ­À´µØ·½
				monsterID = LuaFnCreateMonster( sceneId, monster10ID, x050221_g_monster10Pos[i][1], x050221_g_monster10Pos[i][2], 14, -1, 050221 )
				SetLevel( sceneId, monsterID, mylevel )
				SetCharacterName( sceneId, monsterID, x050221_g_Monster10ID )
			end
