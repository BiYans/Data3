...
x050102_g_LimitMembers = 3					-- ¿ÉÒÔ½ø¸±±¾toÕ ğµ ×îĞ¡¶ÓÎéÈËÊı
...
x050102_g_TakeTimes = 5											-- Ã¿Ìì×î¶àLînh ´ÎÊı
...
function x050102_OnAccept( sceneId, selfId, targetId )
	if IsHaveMission( sceneId, selfId, x050102_g_MissionId ) == 0 then					-- Ã»ÓĞÈÎÎñ²Å¿ÉÒÔ×ßÕâÀï
		...
		if GetLevel( sceneId, selfId ) < Fb2 then
			x050102_NotifyFailBox( sceneId, selfId, targetId, "    ĞÆng c¤p cüa ngß½i quá th¤p, không th¬ chiªn th¡ng. ĞÕt c¤p "..Fb2.." hãy ğªn tìm ta" )
			return
		end
...
function x050102_AcceptEnterCopyScene( sceneId, selfId, targetId )
	if IsHaveMission( sceneId, selfId, x050102_g_MissionId ) > 0 then					-- ÓĞÈÎÎñ²Å¿ÉÒÔ×ßÕâÀï
		...
		if FbTime ~= 0 then
			x050102_NotifyFailBox( sceneId, selfId, targetId, "Còn "..FbTime.." phút næa máy chü s¨ tiªn hành bäo trì, các phø bän ğã ğóng." )
			return
		end
		...
		if nearMemberCount < Fb1 then
			x050102_NotifyFailBox( sceneId, selfId, targetId, "    Lû ğÕo t£c này không phäi v×a ğâu, ngß½i c¥n phäi có 1 s¯ bän lînh và 1 s¯ trş thü, ta m¾i yên tâm. (trong nhóm t¯i thi­u c¥n phäi có "..Fb1.." nhân v§t tr· lên)" )
			return
		end
...
function x050102_OnCopySceneReady( sceneId, destsceneId )
	...
	if validmembercount < Fb1 then
		x050102_NotifyFailTips( sceneId, leaderObjId, "    Lû ğÕo t£c này không phäi v×a ğâu, ngß½i c¥n phäi có 1 s¯ bän lînh và 1 s¯ trş thü, ta m¾i yên tâm. (trong nhóm t¯i thi­u c¥n phäi có "..Fb1.." nhân v§t tr· lên)" )
		return
	end
