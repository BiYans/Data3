...
x232002_g_LimitMembers			= 3		--¿ÉÒÔ½ø¸±±¾toÕ ğµ ×îĞ¡¶ÓÎéÈËÊı
...
x232002_g_Back_X						= 64	--Ô´³¡¾°Î»ÖÃX
x232002_g_Back_Z						= 77	--Ô´³¡¾°Î»ÖÃZ
...
function x232002_CheckAccept( sceneId, selfId, targetId )
	...
	if FbTime ~= 0 then
		x232002_NotifyTip( sceneId, selfId, "Còn "..FbTime.." phút næa máy chü s¨ tiªn hành bäo trì, các phø bän ğã ğóng." )
		return 0
	end
	
	local Fb1, Fb2, Fb3, Fb4 = CallScriptFunction(741307,"FubenChecker",sceneId,selfId,232002,0)
	
	if LuaFnGetLevel( sceneId, selfId ) < Fb2 then
		x232002_NotifyTip( sceneId, selfId, "ĞÆng c¤p cüa các hÕ quá th¤p, phÕm nhân khá lşi hÕi" )
		x232002_NotifyTip( sceneId, selfId, "Vçn ğşi ngß½i ğªn"..Fb2.." sau khi thång c¤p t¾i tìm ta" )
		return 0
	end
	...
	if numMem < Fb1 then
		...
		x232002_NotifyTip( sceneId, selfId, "Chí ít các ngß½i cûng c¥n có "..Fb1.." ta m¾i yên tâm ğ¬ các ngß½i ği" )
		return 0
	end
...
function x232002_OnCopySceneReady( sceneId, destsceneId )
	...
	if numMem < Fb1 then
		x232002_NotifyTip( sceneId, leaderObjId, "Chí ít các ngß½i cûng c¥n có "..Fb2.." ta m¾i yên tâm ğ¬ các ngß½i ği" )
		return
	end
