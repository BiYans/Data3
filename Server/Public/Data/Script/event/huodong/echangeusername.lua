...
function x808008_CallBackChangeUserNameBefore( sceneId, selfId )
	...
	if LuaFnGetAvailableItemCount(sceneId,selfId,30008105) < 1 then
		x808008_MsgBox( sceneId, selfId, "Yêu c¥u Thë Ð±i Tên" )
		return 0
	end
...
function x808008_CallBackChangeUserNameAfter( sceneId, selfId, nRetType )
	...
	if nRetType == 1 then
		...
		TryRecieveItem(sceneId,selfId,30008105,1)
	...
	elseif nRetType == 4 then
		...
		local strText = format("#{_INFOUSR%s} #Hmu¯n làm lÕi cuµc ð¶i, ðã sØ døng Thë Ð±i Tên ð¬ thay ð±i danh ph§n.", playername)
