...
x808032_g_LimitMembers			= 3		--¿ÉÒÔ½ø¸±±¾toÕ ğµ ×îĞ¡¶ÓÎéÈËÊı
...
function x808032_OnEnumerate( sceneId, selfId, targetId )
	...
	if FbTime ~= 0 then
		x808032_NotifyTip( sceneId, selfId, "Còn "..FbTime.." phút næa máy chü s¨ tiªn hành bäo trì, các phø bän ğã ğóng." )
		return
	end

	local Fb1, Fb2, Fb3, Fb4 = CallScriptFunction(741307,"FubenChecker",sceneId,selfId,808032,0)
	local	lev	= GetLevel( sceneId, selfId )
	if lev < Fb2 then
		x808032_NotifyTip( sceneId, selfId, "ĞÆng c¤p cüa ngß½i quá th¤p, c½ bän không l÷t vào m¡t ta. Ğşi sau khi ğÕt c¤p "..Fb2..", hãy t¾i tìm ta" )
		return
	end
	...
	if GetTeamSize( sceneId, selfId ) < Fb1 then
	  x808032_NotifyTip( sceneId, selfId, "Mu¯n thØ thách ta chí ít cûng phäi t¾i "..Fb1.." ngß¶i. Có tí ngß¶i thª này, quá coi thß¶ng ta" )
	  return
	end
	--PrintNum(4)
