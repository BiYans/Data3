...
x806022_g_ScriptId = 806022

x806022_g_GlobalChallengeScript = 806010

-- ¸±±¾³¡¾°
x806022_g_CopyScene = "leitai_2.nav"

-- ¸±±¾³ö¿Ú
x806022_g_CopySceneExit = "leitai_2_area.ini"

-- ¸±±¾µÇÂ½µØ ği¬m
x806022_g_Pos = { x=31, z=32, offset=3.53 }

-- ¸±±¾ÎŞÈËÊ±¹Ø±ÕÊ±¼ä (ºÁ giây)
x806022_g_CopySceneCloseTime = 3000

-- ¸±±¾¶¨Ê±Æ÷´¥·¢ÖÜÆÚ (ºÁ giây)
x806022_g_TickTime = 5000

-- ÀŞÌ¨¿ªÊ¼Ê±¼ä
x806022_g_BeginTick = 3

-- ¸±±¾Ö»Ê£ÏÂmµt ·½Ê±toÕ ğµ ¹Ø±ÕÖÜÆÚ ( µ¥Î»Îª¶¨Ê±Æ÷´¥·¢ÖÜÆÚ )
x806022_g_CloseTick = 3

-- ¸±±¾Pvp¹æÔò
x806022_g_PvpRuler = 9	--zchw

-- ¸±±¾ÀàĞÍºÅ
x806022_g_CopySceneType = FUBEN_PVP_LEITAI

-- Khiêu chiªnË«·½ÕóÓªºÅ
x806022_g_Camp = { self=10, target=11 }

-- Khiêu chiªn½áÊøÒÔºó»Øµ½toÕ ğµ Î»ÖÃ
x806022_g_BackPos = { sceneId=480, x=113, z=172}

-- ÎŞµĞ BUFF ~~~
x806022_g_Buff = 112

x806022_g_BuffID_ClearChgBodyBuff = 84
-- ÈÃ selfId (¼°¶ÓÓÑ) Khiêu chiªn targetId (¼°¶ÓÓÑ)
function x806022_DoChallenge( sceneId, selfId, targetId )
	...
	LuaFnSetSceneLoad_Map( sceneId, x806022_g_CopyScene )						-- µØÍ¼Ğúng±ØĞëÑ¡È¡toÕ ğµ ,¶øÇÒ±ØĞëTÕi Config/SceneInfo.iniÀïÅäÖÃºÃ
	LuaFnSetCopySceneData_TeamLeader( sceneId, CreatorGUID )
	LuaFnSetCopySceneData_NoUserCloseTime( sceneId, x806022_g_CopySceneCloseTime )
	LuaFnSetCopySceneData_PvpRuler( sceneId, x806022_g_PvpRuler )
	LuaFnSetCopySceneData_Timer( sceneId, x806022_g_TickTime )
	LuaFnSetCopySceneData_Param( sceneId, 0, x806022_g_CopySceneType )			-- ÉèÖÃ¸±±¾ÀàĞÍ
	LuaFnSetCopySceneData_Param( sceneId, 1, x806022_g_ScriptId )				-- ½«1ºÅÊı¾İÉèÖÃÎª¸±±¾³¡¾°ÊÂ¼ş½Å±¾ºÅ
	...
	LuaFnSetSceneLoad_Area( sceneId, x806022_g_CopySceneExit )					-- ¼ÓÔØÀŞÌ¨³ö¿Ú
...
function x806022_GetSelfTeamId( sceneId )
	return LuaFnGetCopySceneData_Param( sceneId, 6 ) / 10
end

function x806022_GetTargetTeamId( sceneId )
	return LuaFnGetCopySceneData_Param( sceneId, 7 ) / 10
end

function x806022_GetSelfMembersCount( sceneId )
...
function x806022_ModifySelfMembersCount( sceneId, count )
	local selfInfo = LuaFnGetCopySceneData_Param( sceneId, 6 )
...
function x806022_GetTargetMembersCount( sceneId )
...
function x806022_ModifyTargetMembersCount( sceneId, count )
	local TargetInfo = LuaFnGetCopySceneData_Param( sceneId, 7 )
...
function x806022_CalcPosOffset()
	local offset = x806022_g_Pos.offset * random(100) / 100
...
--¸±±¾ÊÂ¼ş
--**********************************
function x806022_OnCopySceneReady( sceneId, destsceneId )
	...
	for i=0, membersCount-1 do
		...
		if LuaFnIsCanDoScriptLogic( sceneId, members[i] ) == 1 then
			NewWorld( sceneId, members[i], destsceneId, x806022_g_Pos.x + x806022_CalcPosOffset(), x806022_g_Pos.z + x806022_CalcPosOffset())
		end
	end
end

--  ğÕt ğßşc×Ô¼ºÕ¾TÕi ÄÄ·½toÕ ğµ ĞÅÏ¢,1: Khiêu chiªn·½,2: ±»Khiêu chiªn·½
function x806022_GetMySide( sceneId, selfId )
	...
	elseif MyGUID == targetGUID then
		...
		local selfTeamId = x806022_GetSelfTeamId( sceneId )
		local targetTeamId = x806022_GetTargetTeamId( sceneId )
...
--ÓĞÍæ¼Ò½øÈë¸±±¾ÊÂ¼ş
--**********************************
function x806022_OnPlayerEnter( sceneId, selfId )
	...
	local MySide = x806022_GetMySide( sceneId, selfId )
	if MySide == 1 then
		SetUnitCampID( sceneId, selfId, selfId, x806022_g_Camp.self )
		SetPvpAuthorizationFlagByID(sceneId, selfId, 2, 1) --2Ğúng¾º¼¼ÊÚÈ¨±ê¼Ç
	--	x806022_ModifySelfMembersCount( sceneId, 1 )
	else
		SetUnitCampID( sceneId, selfId, selfId, x806022_g_Camp.target )
		SetPvpAuthorizationFlagByID(sceneId, selfId, 2, 1) --2Ğúng¾º¼¼ÊÚÈ¨±ê¼Ç
	--	x806022_ModifyTargetMembersCount( sceneId, 1 )
	end

	-- ¼ÓÉÏ 15  giâytoÕ ğµ  buff,Ò²ĞíÓ¦¸Ã·ÅTÕi  NewWorld ÒÔÇ°,Ö÷Òªº¦ÅÂÈç¹û NewWorld th¤t bÕi¿ÉÄÜ»áÓĞmµt Ğ©¸±×÷ÓÃ
	LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, x806022_g_Buff, 0)

	-- ÉèÖÃÄ¬ÈÏ»¹»ê ği¬mÎª½øÈë³¡¾°toÕ ğµ ÀŞÌ¨
	SetPlayerDefaultReliveInfo( sceneId, selfId, "%10", -1, "0", x806022_g_BackPos.sceneId, x806022_g_BackPos.x, x806022_g_BackPos.z )
	-- ½øÈëĞ£³¡¸±±¾toÕ ğµ Íæ¼ÒÒªÇå³ı±äÉíbuff,·ÀÖ¹²»ÄÜ²Ù×÷±»´òËÀ
	LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, x806022_g_BuffID_ClearChgBodyBuff, 0)
end

-- ¸±±¾³ö¿ÚÀë¿ªµ÷ÓÃCái này º¯Êı
function x806022_LeaveScene( sceneId, selfId )
	-- Í³¼ÆÊ£ÓàÈËÊı
	--local MySide = x806022_GetMySide( sceneId, selfId )
	--if MySide == 1 then
	--	x806022_ModifySelfMembersCount( sceneId, -1 )
	...
	CallScriptFunction( (400900), "TransferFunc", sceneId, selfId, x806022_g_BackPos.sceneId, x806022_g_BackPos.x, x806022_g_BackPos.z )
end

--**********************************
--ÓĞÍæ¼ÒTÕi ¸±±¾ÖĞËÀÍöÊÂ¼ş
--**********************************
function x806022_OnHumanDie( sceneId, selfId, killerId )
end

--**********************************
--¸±±¾³¡¾°¶¨Ê±Æ÷ÊÂ¼ş
--**********************************
function x806022_OnCopySceneTimer( sceneId, nowTime )
	-- ¸±±¾Ê±ÖÓÉèÖÃ
	...
	if leaveFlag == 1 then																			-- C¥n Àë¿ª
		...
		if x806022_g_CloseTick <= leaveTickCount then
			-- ½«µ±Ç°¸±±¾³¡¾°ÀïtoÕ ğµ ËùÓĞÈË´«ËÍ»ØÔ­À´½øÈëÊ±ºòtoÕ ğµ ³¡¾°
			local membercount = LuaFnGetCopyScene_HumanCount( sceneId )
			for	i=0, membercount-1 do
				local playerId = LuaFnGetCopyScene_HumanObjId( sceneId, i )
				if LuaFnIsObjValid( sceneId, playerId ) == 1 and LuaFnIsCanDoScriptLogic( sceneId, playerId ) == 1 then
					x806022_LeaveScene( sceneId, playerId )
				end
			end
		else
			-- Í¨ÖªÊ£ÓàÍæ¼Ò¸±±¾¼´½«¹Ø±Õ
			local membercount = LuaFnGetCopyScene_HumanCount( sceneId )
	  		local strText = format( "Lôi ğài s¨ ğóng cØa trong vòng %d giây næa", (x806022_g_CloseTick-leaveTickCount)*x806022_g_TickTime/1000 )
	  		BeginEvent( sceneId )
	...
	elseif tick > x806022_g_BeginTick then
		...
		for	i=0, membercount-1 do
			local playerId = LuaFnGetCopyScene_HumanObjId( sceneId, i )
			if LuaFnIsObjValid( sceneId, playerId ) == 1 and LuaFnIsCanDoScriptLogic( sceneId, playerId ) == 1 then
	  		if x806022_GetMySide( sceneId, playerId ) == 1 then
		...
		if tick < x806022_g_BeginTick then
			strText = format( "Thi ğ¤u s¨ b¡t ğ¥u trong %d giây næa", (x806022_g_BeginTick-tick)*x806022_g_TickTime/1000 )
		else
