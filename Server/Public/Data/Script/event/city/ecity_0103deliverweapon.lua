...
x600005_g_MissionTarget = "    Ğem %i ğßa cho %n"							--Møc tiêu nhi®m vø
x600005_g_ContinueInfo = "    Nhi®m vø cüa các hÕ vçn chßa hoàn thành à?"					--Î´Hoàn t¤t nhi®m vøtoÕ ğµ npc¶Ô»°
x600005_g_SubmitInfo = "    Sñ tình tiªn tri¬n nhß thª nào r°i?"								--Íê³ÉÎ´Ìá½»Ê±toÕ ğµ npc¶Ô»°
x600005_g_MissionComplete = "    Làm t¯t l¡m, r¤t t¯t r¤t t¯t. "					--Hoàn t¤t nhi®m vønpcËµ»°toÕ ğµ »°

x600005_g_StrForePart = 5
x600005_g_ItemOffset = 18												-- Suppose to 18, ±íÀïµÚ¼¸cáiÎïÆ·ÁĞtoÕ ğµ Æ«ÒÆÁ¿
x600005_g_NPCOffset = 7													-- Suppose to 7, ±íÀïµÚ¼¸ÁĞ NPC toÕ ğµ Æ«ÒÆÁ¿
x600005_g_NPCOffsetEx = 245													-- Suppose to 7, ±íÀïµÚ¼¸ÁĞ NPC toÕ ğµ Æ«ÒÆÁ¿
...
--ÈÎÎñÈë¿Úº¯Êı
--**********************************
function x600005_OnDefaultEvent( sceneId, selfId, targetId )	-- ği¬m»÷¸ÃÈÎÎñºóÖ´ĞĞ´Ë½Å±¾
	if GetName( sceneId, targetId ) ~= x600005_g_Name then		--ÅĞ¶Ï¸ÃnpcĞúng·ñĞúng¶ÔÓ¦ÈÎÎñtoÕ ğµ npc
		return
	end
...
function x600005_OnAccept( sceneId, selfId, targetId )
	...
	if GetLevel( sceneId, selfId ) < 20 then
		CallScriptFunction( x600005_g_CityMissionScript, "NotifyFailTips", sceneId, selfId, "Không ğü ğÆng c¤p" )
		return
	end
	...
	if pos == -1 then
		CallScriptFunction( x600005_g_CityMissionScript, "NotifyFailTips", sceneId, selfId, "Ch² tr¯ng trong túi xách không ğü" )
		return
	end
	...
	SetMissionEvent( sceneId, selfId, x600005_g_MissionId, 4 ) -- ×¢²á x600005_OnLockedTarget ÊÂ¼ş
	...
	BeginEvent( sceneId )
		...
		if rand == 1 then
			missionInfo = format( "    Chúng ta vì %s %s (%d, %d) ğã ğúc thành #Y%s#W, hy v÷ng các hÕ có th¬ giúp ta chuy¬n giúp.",
				strNpcScene, strNpcName, nPosX, nPosZ, strItemName )
		else
			missionInfo = format( "    %s %s (%d, %d) ğã ğ£t #Y%s#W · ğây, hy v÷ng các hÕ có th¬ chuy¬n cho %s",
				strNpcScene, strNpcName, nPosX, nPosZ, strItemName, x600005_g_Pronoun[nGender] )
		end

		AddText( sceneId, missionInfo )
		AddText( sceneId, "#r    Các hÕ ğã nh§n " .. x600005_g_MissionName )
	EndEvent( sceneId )
...
--É±ËÀ¹ÖÎï»òÍæ¼Ò
--**********************************
function x600005_OnKillObject( sceneId, selfId, objdataId, objId )	--²ÎÊıÒâË¼: ³¡¾°ºÅ¡¢Íæ¼ÒobjId¡¢¹ÖÎï±íÎ»ÖÃºÅ¡¢¹ÖÎïobjId
end
...
function x600005_OnLockedTarget( sceneId, selfId, objId )
	...
	local misIndex = GetMissionIndexByID( sceneId, selfId, x600005_g_MissionId )		-- ği¬mµ½ÈÎÎñTÕi 20cáiÈÎÎñÖĞtoÕ ğµ ĞòÁĞºÅ
	...
	if GetName( sceneId, objId ) == strNpcName then
		...
		if DelItem( sceneId, selfId, nItemId, 1 ) == 1 then
			...
			-- ½«¸ÃÈÎÎñ´Ó x600005_OnLockedTarget ÊÂ¼şÁĞ±íÖĞÇå³ı,¸ÃÈÎÎñ²»ÔÙ¹ØĞÄ´ËÀàÊÂ¼ş
			ResetMissionEvent( sceneId, selfId, x600005_g_MissionId, 4 )
			TAddText( sceneId, "    Danh tín cüa quı bang th§t t¯t, vô cùng cäm ½n." )
		end
	end
end
