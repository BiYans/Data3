...
function x402263_CheckCanEnter( sceneId, selfId, targetId )
	...
	--ÈËÊýÐúng·ñ¹»....
	local Fb1, Fb2, Fb3, Fb4 = CallScriptFunction(741307,"FubenChecker",sceneId,selfId,402263,0)

	if GetTeamSize(sceneId,selfId) < Fb1 then
		return 0, "    Ðµi ngû cüa các hÕ vçn còn quá ít thành viên, t¯t h½n nên l§p t± ðµi trên "..Fb1.." ngß¶i hãy ðªn ðây g£p ta."
	end
	...
	for i=0, NearTeamSize-1 do
		...
		if GetLevel( sceneId, PlayerId ) < Fb2 then
	...
	if nHumanNum > 0 then
		...
		msg = msg .. Humanlist[nHumanNum-1] .. " c¤p ðµ th¤p h½n "..Fb2..", không th¬ tham gia!"
		return 0, msg
	...
	if nHumanNum > 0 then
		...
		msg = msg .. Humanlist[nHumanNum-1] .. " ngày hôm nay ðã "..Fb4.." l¥n khiêu chiªn Phiêu Mi¬u Phong."
		return 0, msg
