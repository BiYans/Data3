...
function WuhunExtraPropertyDel_LoadOption()
	...
	if WuhunAuthor ~= "No_Author" then
		local VoHon_Base = tonumber(string.sub(WuhunAuthor,13,13)) - 5;
		for i = 1, 10 do
...
function WuhunExtraPropertyDel_OnEvent(event)
	if (event == "UI_COMMAND" and tonumber(arg0) == 264001) then
...
function WuhunExtraPropertyDel_OK_Clicked()
	...
	Clear_XSCRIPT()
		...
		Set_XSCRIPT_Parameter(0, 898) --8 for pos, 01 for function--
