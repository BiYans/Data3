...
function PetHuanhua_OnEvent(event)
  ...
  elseif event == "UI_COMMAND" and tonumber( arg0 ) == 20170123 and this : IsVisible() then
...
function PetHuanhua_OK_Clicked()
	...
	if m_Comfirmed == 0 then
		...
		if (m_KeepModel == 1) then
    	ShowSystemInfo("RXZS_XML_33")
		else
    	ShowSystemInfo("RXZS_XML_34")
		end
		m_Comfirmed = 1
	else
		Clear_XSCRIPT()
...
function PetHuanhua_Pet_Show()
	...
	local transfomerID = 1500;--Pet:GetHHChangeModel(m_PetIndex)
	if transfomerID ~= nil and transfomerID > 0 then
		--Pet:HuanhuaOpenPetJian(m_PetIndex,0);
    Pet:PetOpenPetJian(m_PetIndex,"self");
	end
end
...
function PetHuanhua_OnSelectPet(petIndex)
	...
	--类型不符
	local isCan = 1;--Pet:EnableHuanhua(petIndex)
	if isCan ~= 1 then
...
function PetHuanhua_UICheck()
	...
	if  m_PetIndex ~= -1 then
		...
		m_needMoney = 50000;--Pet:GetHuanhuaMoney(m_PetIndex)
		...
		if isMoneyEnough == 1 and m_KeepModel ~= -1 then
