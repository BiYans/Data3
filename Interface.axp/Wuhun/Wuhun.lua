...
function Wuhun_OnEvent(event)
	...
	if event == "UI_COMMAND" and tonumber(arg0) == 74130101 then
		VoHon_Author = Get_XParam_STR(1)
		
		--PushDebugMessage(VoHon_Author);
	...
	--elseif( event == "TOGGLE_WUHUN_PAGE") then
	elseif (event == "UI_COMMAND" and tonumber(arg0) == 20111211) then
	...
	elseif( event == "REFRESH_EQUIP" and this:IsVisible() ) then
...
function Wuhun_Update()
	...
	local ActionKFS = EnumAction(8,"equip")
	...
	if VoHon_Author == "NoWuhun" then
		...
		--Life
		Wuhun_Life_Text1:SetText("#cfff263Th÷ m®nh:")
		...
		--AttrEx
		local slot = 0;--DataPool:GetKfsData("SLOT")
		...
		--ExtraLevel
		local KFS_ExtraLevel = tonumber(string.sub(VoHon_Author,16,16))
		Wuhun_ExtraLevel_Text:SetText(KFS_ExtraLevel)
		
		--Model + Magic
		Wuhun_FakeObject:SetFakeObject("");
		Magic = tonumber(string.sub(VoHon_Author,57,57))
		if ItemID == 10308001 or ItemID == 10308003 then
			Player:SetHorseModel(1080 + Magic);
			Wuhun_FakeObject:SetFakeObject("My_Horse");
		end
		
		if ItemID == 10308002 or ItemID == 10308004 then
			Player:SetHorseModel(1085 + Magic);
			Wuhun_FakeObject:SetFakeObject("My_Horse");
		end
		
		if Magic >= 1 then
			Kfs_Magic:SetProperty("NormalImage" , Kfs_Magic_Image[Magic] )
			Kfs_Magic:SetToolTip(Kfs_Magic_tips[Magic+1])
		else
			Kfs_Magic:SetProperty("NormalImage" , "set:Wuhun4 image:Wuhun4_5" )
			Kfs_Magic:SetToolTip(Kfs_Magic_tips[1])
		end

		--NeedLv
		Wuhun_NeedLevel_Text:SetText("65")

		--Lv
		local KFS_Level = tonumber(string.sub(VoHon_Author,5,7));
		Wuhun_Level_Text:SetText(KFS_Level)

		--AttactType
		if ItemID == 10308001 or ItemID == 10308003 then
			Wuhun_Type:SetText("#cfff263LoÕi cß¶ng lñc")
		elseif ItemID == 10308002 or ItemID == 10308004 then
			Wuhun_Type:SetText("#cfff263LoÕi nµi lñc")
		end
		
		--Life
		Wuhun_Life_Text1:SetText("#cfff263Th÷ m®nh: 299/300")

		--Exp
		Wuhun_Exp:SetText("#cfff263Kinh nghi®m: "..tonumber(string.sub(VoHon_Author,8,12)).."/"..VoHon_Exp[KFS_Level][1])

		--GrowRate
		local KFS_GrowRate = tonumber(string.sub(VoHon_Author,13,15))
		if KFS_GrowRate >= 500 and KFS_GrowRate <= 599 then
			Wuhun_Growth1:SetText("#cfff263Trß·ng thành: "..KFS_GrowRate.." (S½ c¤p)")
		end
		
		if KFS_GrowRate >= 600 and KFS_GrowRate <= 699 then
			Wuhun_Growth1:SetText("#cfff263Trß·ng thành: "..KFS_GrowRate.." (Xu¤t s¡c)")
		end
		
		if KFS_GrowRate >= 700 and KFS_GrowRate <= 799 then
			Wuhun_Growth1:SetText("#cfff263Trß·ng thành: "..KFS_GrowRate.." (Ki®t xu¤t)")
		end
		
		if KFS_GrowRate >= 800 and KFS_GrowRate <= 899 then
			Wuhun_Growth1:SetText("#cfff263Trß·ng thành: "..KFS_GrowRate.." (Trác vi®t)")
		end
		
		if KFS_GrowRate >= 900 and KFS_GrowRate <= 999 then
			Wuhun_Growth1:SetText("#cfff263Trß·ng thành: "..KFS_GrowRate.."(Toàn mÛ)")
		end
		...
		if ItemID == 10308001 or ItemID == 10308003 then
			for i = 1, 5 do
				...
				Kfs_Base_Original_Value[i]:SetText(Property_Value)
				
				Property_Value = VoHon_Property[KFS_Level][KFS_Base*5 + i]
				Kfs_Base_Value[i]:SetText(Property_Value)
			end
		else
			Property_Value = VoHon_Property[KFS_Level][1]
			Kfs_Base_Original_Value[2]:SetText(Property_Value)
			Property_Value = VoHon_Property[KFS_Level][KFS_Base*5 + 1]
			Kfs_Base_Value[2]:SetText(Property_Value)
			
			Property_Value = VoHon_Property[KFS_Level][2]
			Kfs_Base_Original_Value[1]:SetText(Property_Value)
			Property_Value = VoHon_Property[KFS_Level][KFS_Base*5 + 2]
			Kfs_Base_Value[1]:SetText(Property_Value)
			for i = 3, 5 do
				Property_Value = VoHon_Property[KFS_Level][i]
				Kfs_Base_Original_Value[i]:SetText(Property_Value)
		...
		for i = 1, nSumSkill do
			...
			if theAction:GetDefineID() == KFS_Skill1 then
				Kfs_Skills[1]:SetActionItem(theAction:GetID());
			elseif theAction:GetDefineID() == KFS_Skill2 then
				Kfs_Skills[2]:SetActionItem(theAction:GetID());
			elseif theAction:GetDefineID() == KFS_Skill3 then
