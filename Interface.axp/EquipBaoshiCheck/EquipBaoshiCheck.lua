...
local g_EquipBaoshiCheck_Equip_Gem            = {}     --- 装备上的宝石
local g_EquipBaoshiCheck_Equip_GemTypeList    = {}     --- 装备上的宝石类型信息

local g_EquipBaoshiCheck_Gem_Container              = {}     --- 装备位上的宝石
local g_EquipBaoshiCheck_Gem_Container_GemTypeList  = {}     --- 装备位上的宝石类型信息

local g_EquipBaoshiCheck_IsHaveEquiped  = -1     --- 是否有装备

local g_EquipBaoshiCheck_Current_Sel = -1       --- 纪录当前选择的哪个界面

--经典版的图片素材
local g_EquipBaoshiCheck_UnActived_Image  = "set:Agname3 image:BaoShiCheckFeng"    --- 未打孔
local g_EquipBaoshiCheck_Actived_Image    = "set:UIIcons image:Equip_Ring"      --- 暂时使用真元激活的图片
...
local g_EquipBaoshiCheck_Equip_TypeList =
{
	...
	[17] = { name="#{BHZB_140521_241}", gemConIndex=16, equipIndex=20, },    --- 符石传说
...
function EquipBaoshiCheck_OnLoad()
	...
	if SystemSetup:IsClassic() == 0 then	--若客户端是唯美版，则重新设置图片素材为唯美版
		g_EquipBaoshiCheck_UnActived_Image  = "set:WM_CommonFrame1 image:Feng"    --- 未打孔
		g_EquipBaoshiCheck_Actived_Image    = "set:WM_Button2 image:Equip_Ring"      --- 暂时使用真元激活的图?
	end
...
function EquipBaoshiCheck_Update( btnIndex )
    ...
    if updateRet then
        ...
        for i=1, 4 do
            ...
            --- 没有宝石
                ...
                if i > holeCount then
                    g_EquipBaoshiCheck_Equip_Gem[i] : SetActionItem( -1 );
                    g_EquipBaoshiCheck_Equip_Gem[i] : SetToolTip( "#{WBSS_120727_40}" )   --- 装备的该孔位未开启
                    ...
                    if i == 4 then
                        ...
                        if 0 == EquipBaoshiCheck_Can_Stiletto_Four( itemTblIdx, bIsDark, bIsHxy ) then
                            g_EquipBaoshiCheck_Equip_Gem[i] : SetToolTip( "#{WBSS_120727_41}" )   --- 装备无法开启第四孔
                        end
                    end
                --- 开孔未镶嵌宝石则对应格子处显示空格子
                else
                    g_EquipBaoshiCheck_Equip_Gem[i] : SetProperty( "BackImage",  g_EquipBaoshiCheck_Actived_Image )
                    g_EquipBaoshiCheck_Equip_Gem[i] : SetToolTip( "#{WBSS_120727_45}" )   --- 该孔位未镶嵌宝石
                end
            end
    ...
    for i = 1, 4 do
        ...
        if itemID ~= -1 then
            ...
            g_EquipBaoshiCheck_Gem_Container[i] : SetToolTip( "#{WBSS_120727_45}" )   --- 该孔位未镶嵌宝石
        end
    end
...
function EquipBaoshiCheck_UpdateHints( typeListIndex )
    ...
    if ret == 1 then
        EquipBaoshiCheck_Frame_Info : SetText( "#{WBSS_120727_20}" )      --- 装备位上无装备，无法进行万宝随身互换
    elseif ret == 2 then
        EquipBaoshiCheck_Frame_Info : SetText( "#{WBSS_120727_98}" )      --- 装备位上尚未镶嵌宝石，该装备位将不能进行互换
    elseif ret == 3 then
        EquipBaoshiCheck_Frame_Info : SetText( "#{WBSS_120727_22}" )      --- 对应的装备孔位中没有宝石，无法进行万宝随身互换
    elseif ret == 4 then
        EquipBaoshiCheck_Frame_Info : SetText( "#{WBSS_120727_23}" )      --- 互换时装备前三孔会出现同类型宝石，无法进行万宝随身互换
    elseif ret == 0 then
        EquipBaoshiCheck_Frame_Info : SetText( "#{WBSS_120727_19}" )      --- 满足互换条件，可以进行万宝随身互换
    else
        EquipBaoshiCheck_Frame_Info : SetText( "" )
    end
end
