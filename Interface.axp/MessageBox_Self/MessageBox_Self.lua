...
local FrameInfoList = {
	...
	DELETE_FRIEND_MESSAGE		= 25,	--È·¶¨É¾³ýºÃÓÑµÄÌáÊ¾¿ò
		...
		GUILD_DIS_FIRSTMAN    =87,  --È·ÈÏÉ¾³ýµÚÒ»¼Ì³ÐÈË  
    ...
    GUILD_LEAGUE_CREATE_CONFIRM = 221,		--	´´½¨°ï»áÍ¬ÃËÈ·ÈÏ
		...
		DART_ADJUST = 227,	-- °µÆ÷ÊôÐÔµ÷Õû   Vega 20090422
		TRUST_FRIEND = 228,
...
local Dart_Data = {}			--°µÆ÷Êý¾Ý

local NeedUseConfirmItemData = {}	--ÐèÒªÊ¹ÓÃÈ·ÈÏµÄÎïÆ·Êý¾Ý
...
function MessageBox_Self_OnEvent(event)
	...
	if ( event == "SET_GUILD_FIRSTMAN_NAME" ) then
		...
		if szMsg ~= "" then
			MessageBox_Self_Text:SetText( "#{BHCR_090713_08}"..szMsg);	-- ÉèÖÃÄÚÈÝ	
		else
...
--	end
	...
	elseif ( event == "UI_COMMAND" ) then
		...
		elseif (tonumber(arg0) == 805048 ) then
			...
			if POS_GUILD_CHIEF ~= myPos	then
				PushDebugMessage("#{BHCR_090713_06}") --·Ç°ïÖ÷²»ÄÜ³·ÏúµÚÒ»¼Ì³ÐÈË
				this:Hide();
				return
			else
	...
	elseif( event == "NEED_USE_CONFIRM_ITEM" ) then
		...
		NeedUseConfirmItemData[4] = arg3; --ÎïÆ·°üÄÚË÷Òý
		NeedUseConfirmItemData[5] = arg4; --ÎïÆ·±íÄÚË÷Òý
	...
	if ( event == "TRUST_FRIEND_OPEN_DEL_CHECK" ) then
		...
		MessageBox_Self_Text:SetText( "#{XRHB_09515_15}"..arg1.."#{XRHB_09515_16}");	-- ÉèÖÃÄÚÈÝ		
...
function MessageBox_Self_OK_Clicked_Ex()
	...
	if( FrameInfoList.NEED_USE_CONFIRM_ITEM == g_FrameInfo ) then
		...
		tonumber(NeedUseConfirmItemData[4]), --ÎïÆ·°üÄÚË÷Òý
		tonumber(NeedUseConfirmItemData[5])  --ÎïÆ·±íÄÚË÷Òý
		);
	end
...
function MessageBox_Self_ShowDart()
	...
	elseif (Dart_Data[1] == 6) then
		MessageBox_Self_Text:SetText( "#{FBSJ_090421_1}" ); --ÍüÎÞÊ¯
	...
	elseif (Dart_Data[1] == 9) then
		MessageBox_Self_Text:SetText( "#{FBSJ_090421_3}" ); --Ô¡»ðÊ¯
	end
...
function MessageBox_Self_AdjustDart()
	...
	elseif (Dart_Data[1] == 9) then
		DataPool:DarkReset(Dart_Data[2], 1);		--Ô¡»ðÊ¯
	end
end
