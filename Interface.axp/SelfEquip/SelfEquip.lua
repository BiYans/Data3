...
local g_AutoClick_BtnFlag = -1			-- è®°å½•å½“å‰?é¼ æ ‡å·¦é”®æ˜¯åœ¨å“ªä¸ªæŒ‰é’®æŒ‰ä¸‹
local g_AutoClickTimer_Step = 50		-- å¤šå°‘æ—¶é—´(æ¯«ç§’)æ¨¡æ‹Ÿä¸€æ¬¡ Click æ“?ä½œ
local g_AutoClick_FunList = {}			-- å°†å…¬ç”¨ä¸€ä¸ª Timer çš„å›žè°ƒåŠŸèƒ½å‡½æ•°æ”¾åˆ°ä¸€ä¸ªæ•°ç»„
local g_AutoClick_Going = -1			-- æ ‡å¿—æ˜¯å?¦å¼€å§‹è‡ªåŠ¨ç‚¹å‡»æ“?ä½œ(ç¬¬ä¸€æ¬¡LButtonå?Žç»?è¿‡Xä¸ªTimeræ‰?ç®—å¼€å§‹, ä¹Ÿå°±æ˜¯è¯´æ˜¯ g_AutoClickTimer_Step * X çš„æ—¶å€™å¼€å§‹è¿›è¡Œè‡ªåŠ¨åŠ , è¿™æ ·ä¸ºäº†é˜²æ­¢æœ¬æ?¥è¦?ç‚¹å‡»ä¸€ä¸‹çš„ç»“æžœç‚¹äº†å¥½å¤šä¸‹)
...
function SelfEquip_SetStateTooltip()
	...
	SelfEquip_IceFastness:SetToolTip("Bång công: "..tostring(iIceAttack).."#rKháng Bång: "..tostring(iIceDefine).."#rGiäm kháng Bång: "..tostring(iIceResistOther).."#rGiäm âm kháng Bång: "..GKA_Bang );
	SelfEquip_FireFastness:SetToolTip("Höa công: "..tostring(iFireAttack).."#rKháng Höa: "..tostring(iFireDefine).."#rGiäm kháng Höa: "..tostring(iFireResistOther).."#rGiäm âm kháng Höa: "..GKA_Hoa );
	SelfEquip_ThunderFastness:SetToolTip("Huy«n công: "..tostring(iThunderAttack).."#rKháng Huy«n: "..tostring(iThunderDefine).."#rGiäm kháng Huy«n: "..tostring(iThunderResistOther).."#rGiäm âm kháng Huy«n: "..GKA_Huyen );
	SelfEquip_PoisonFastness:SetToolTip("Ðµc công: "..tostring(iPoisonAttack).."#rKháng Ðµc: "..tostring(iPoisonDefine).."#rGiäm kháng Ðµc: "..tostring(iPoisonResistOther).."#rGiäm âm kháng Ðµc: "..GKA_Doc );
...
function SelfEquip_UpdateMaskByIndex( index )
  ...
  if (m_ZhiZunAnimate[ index ] ~= nil) then
    ...
    if (tonumber(ActionIndex:GetID()) ~= 0) then
      if index == 16 or index == 17 then			--8:Æï³Ë 9:ÐÐÄÒ 10:Ïä×Ó
        if index == 17 then
...
--***************************************************
-- å®šæ—¶å™¨å›žè°ƒå‡½æ•°
--    å®žçŽ°æ…¢å?¯åŠ¨, ä»¥å?Žå?¯ä»¥è€ƒè™‘åŠ é€Ÿ(å¿…è¦?æ€§ä¸?å¤§)
--***************************************************
function SelfEquip_AutoClick_Timer()
	if (g_AutoClick_BtnFlag ~= -1) then
		-- ç¬¬ä¸€æ¬¡LButtonå?Žç»?è¿‡Xä¸ªTimeræ‰?ç®—å¼€å§‹, ä¹Ÿå°±æ˜¯è¯´æ˜¯ g_AutoClickTimer_Step * X çš„æ—¶å€™å¼€å§‹è¿›è¡Œè‡ªåŠ¨åŠ , è¿™æ ·ä¸ºäº†é˜²æ­¢æœ¬æ?¥è¦?ç‚¹å‡»ä¸€ä¸‹çš„ç»“æžœç‚¹äº†å¥½å¤šä¸‹
		if (g_AutoClick_Going < 4) then
			g_AutoClick_Going = g_AutoClick_Going + 1
			--ç›®å‰?å…ˆè®¾ç½® 6 Step çš„ç­‰å¾…æ—¶é—´, ä¸‹é?¢æ³¨é‡Šçš„ä»£ç ?å?¯ä»¥å?Žæ?¥ç”¨äºŽå®žçŽ°æ…¢å?¯åŠ¨, é€?æ¸?åŠ é€Ÿæ•ˆæžœ.
...
--***************************************************
-- é¼ æ ‡å·¦é”®æ?¾å¼€æ“?ä½œ
--    æ³¨æ„?è¿™é‡Œå…¶å®žæ˜¯ä»£æ›¿ Click, æ‰€ä»¥éœ€è¦?æ‰§è¡Œä¸€æ¬¡ Click æ“?ä½œ
--***************************************************
