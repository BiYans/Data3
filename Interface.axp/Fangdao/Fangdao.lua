...
function Fangdao_InitDlg()
	...
	Fangdao_WP2jiesuo:SetText("#{DHMB_90417_12}");
	...
	local isPhoneProtect = 0;--IsPhoneProtectSetup();
	if(tonumber(isPhoneProtect) == 1) then
		Fangdao_TelMibaoShezhi:SetText("Ðã c¯ ð¸nh");
		Fangdao_TelMibaoShezhi:Disable();
	else
		Fangdao_TelMibaoShezhi:SetText("Chßa c¯ ð¸nh");
		Fangdao_TelMibaoShezhi:Enable();
	end
...
function Fangdao_TelMibaoShezhi_Clicked()
	Fangdao_Close();
	OpenDianhuaMibao();
end

function Fangdao_TelMibao_Clicked()
	Fangdao_Close();
	OpenDianhuaMibao();
end
