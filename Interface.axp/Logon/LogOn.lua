...
function LoginLogOn_OnLoad()
  ...
  local TailCount = 6
	...
	--µç»°ÃÜ±£·ÅÔÚÓÎÏ·ÖÐÁË£¬ËùÒÔÏÂÃæµÄ¾ÍÌø¹ýÈ¥ÁË£¬houzhifang 090417
	...
	-- ´ò¿ªÕÊºÅÊäÈë½çÃæ
 	if( event == "GAMELOGIN_OPEN_COUNT_INPUT" ) then
	  ...
	  if bGameMiBao == nil or bGameMiBao == "" then
		...
		LogOn_PassWord_Protect_Check:SetCheck(tonumber(bGameMiBao))
		...
		-- 55928 ¸ù¾Ý²âÊÔÒâ¼û£¬½«Ä¬ÈÏµÄÈí¼üÅÌÊäÈë½¹µã´ÓÃÜÂë¿ò¸Äµ½ÕËºÅÊäÈë¿ò
...
function LogOn_CheckAccount()
	...
	if(strName ~="" and strPassword == "") then
		Logon_ID_TabPressed();
		return;
	elseif( strPassword ~="" and strName == "" ) then
		Logon_Password_TabPressed();
		return;
	elseif( strPassword == "" and strName =="" ) then
		return;
	end
...
function LogOn_AntiDetect_MouseEnter()
	...
	LogOn_Info:SetText("Nh¤n b¡t ð¥u scan Trojan");
