...
function PS_ShopMag_OnEvent(event)
	if ( event == "PS_OPEN_MY_SHOP" )   then
		...
		local actIndex = g_nCurStallIndex-1;--PlayerShop:UIIndexToLogicIndex(g_nCurStallIndex-1, true)
	...
	elseif( event == "UI_COMMAND" )    then
    	...
    	elseif( tonumber(arg0) == 701800 ) then
				-- ¸üÐÂËø¶¨×´Ì¬
				local lockStatus = 0;--Get_XParam_INT( 0 );
				local leftTime = 0;--Get_XParam_INT( 1 );
...
function PS_ShopMag_ShowHide_Windows()
	...
	if( g_SaleOuting == 1) then --´¦ÓÚÅÌ³ö×´Ì¬
		...
		local lockStatus = 0;--PlayerShop : GetLockStatus( );
...
function PS_ShopMag_Curtail_Click()
	local actIndex = 0;--PlayerShop:UIIndexToLogicIndex(g_nCurStallIndex-1, true)
	if( actIndex ~= -1 ) then
...
function PS_Update_LockStatus( )
	...
	local lockStatus = 0;--PlayerShop : GetLockStatus();
	local leftTime = 0;--PlayerShop : GetUnlockTime();
	...
	elseif ( lockStatus == STATUS_UNLOCKING ) then		-- ÕýÔÚ½âËø
		...
		PS_ShopMag_Stall_StateSuo2 : SetText( "#{SDS_90511_22}"..leftDay.."#{SDS_90511_6}"..lastDayLeftHour.."#{SDS_90511_25}" );
	end
